...
The 2x textures are far too large to vendor, so they ship as a GitHub release
asset. `TEXTURE_VERSION` below pins which one this checkout expects: the
...
TEXTURE_VERSION = "0.0.4"
TEXTURE_SHA256 = "0fdc72281420d84e7f08c080e74b682686ca3320fccad97d186f5a5945453965"
TEXTURE_URL = (
...
def fetch_package() -> str:
    ...
    name = "civbe-4k-textures-v%s.7z" % TEXTURE_VERSION
...
def install_textures(game: str, force: bool) -> None:
    ...
    # 7z has no tarfile-style traversal filter, but the archive is SHA-256
    # pinned above, so its contents are exactly what package-textures.sh built.
    subprocess.run(["7z", "x", "-y", "-o" + game, pkg],
