...
local ListingBoxDefaultColor	= 0x90c8c8c8;	-- Listings box default color.
local ListingBoxHighlightColor	= 0xe1e1e1e1;	-- Listings box highlight color.
local EnableText				= Locale.ConvertTextKey("TXT_KEY_MODDING_ENABLEMOD");
local DisableText				= Locale.ConvertTextKey("TXT_KEY_MODDING_DISABLEMOD");
...
function RefreshMods()
	...
	Controls.ListingStack:DestroyAllChildren();
	...
	for i,modinfo in ipairs(g_SortedMods) do
		...
		local listing = {};
		ContextPtr:BuildInstanceForControl( "ListingButtonInstance", listing, Controls.ListingStack );
		...
		if(modinfo.State == "Installed" or modinfo.State == "NeedsUpdate") then
			...
			enableBtn:SetHide( false );
			enableBtn:SetDisabled(not modinfo.Enabled);
...
function ShowInstalledModDetails(modId, modVersion)
	...
	local newlinedDetails = string.gsub(modDetails.Description, "\r\n", "\n" );
	newlinedDetails = string.gsub(newlinedDetails, "\n", "[NEWLINE]");
	Controls.SelectedModDescription:SetText(newlinedDetails);
	...
	if(bCanUnsubscribe) then
		LuaEvents.ModBrowserSetDeleteButtonState(true, true, Locale.Lookup("{TXT_KEY_MODDING_UNSUBSCRIBE_MOD:upper}"));
	elseif(bCanDeleteMod) then
		LuaEvents.ModBrowserSetDeleteButtonState(true, true, Locale.Lookup("{TXT_KEY_MODDING_DELETEMOD:upper}"));
	else
		LuaEvents.ModBrowserSetDeleteButtonState(false, false);
	end
...
-- ===========================================================================
function OnRefreshMods()
	RefreshMods();
end
LuaEvents.RefreshMods.Add( OnRefreshMods )
