...
function GetDefaultMaxTurns()
	-- Returns the default max turns for the current game speed.
	local defaultMaxTurns = 0;
	local gameSpeedType = GameInfo.GameSpeeds[PreGame.GetGameSpeed()].Type;
	for row in DB.Query([[SELECT SUM(TurnsPerIncrement) AS MaxTurns FROM GameSpeed_Turns WHERE GameSpeedType = ?]], gameSpeedType) do
		defaultMaxTurns = row.MaxTurns;
	end
...
function UpdateGameOptionsDisplay(bUpdateOnly)
	...
	Controls.MapTerrainPullDown:GetButton():SetDisabled( not bCanEdit );
...
----------------------------------------------------------------
-- MAP TERRAIN
function OnMapTerrainSelect(id)
	PreGame.SetPlanet( id );
	UpdateGameOptionsDisplay();
end

function PopulateMapTerrainPulldown()
	...
	pullDown:CalculateInternals();
	pullDown:RegisterSelectionCallback(OnMapTerrainSelect);
end
...
function OnMaxTurnsEditBoxChange()	
	...
	PreGame.SetInitialMaxTurns( Controls.MaxTurnsEdit:GetText() );
	SendGameOptionChanged();
end
...
function OnMaxTurnsChecked()
	...
	PreGame.SetInitialMaxTurns(PreGame.GetMaxTurns());
...
function RefreshGameOptions()
	...
	for row in GameInfo.Victories() do
		...
		if (bCanEdit) then
			...
			victoryCondition.GameOptionRoot:SetAlpha( 0.5 );
		end
		
		if (row.Permanent) then 
			-- User is not able to change "permanent" victory types. (Time victory)
			victoryCondition.GameOptionRoot:SetDisabled(true);
			victoryCondition.GameOptionRoot:SetAlpha( 0.0 );	--HACK for ZBR, hiding the Time victory entirely
		else
			victoryCondition.GameOptionRoot:SetAlpha( 1.0 );
		end
	...
	for option in GameInfo.GameOptions{Visible = 1} do
		...
		if( option.Type ~= "GAMEOPTION_PITBOSS" -- just don't show pitboss option for now.
				and option.Type ~= "GAMEOPTION_END_TURN_TIMER_ENABLED" 
