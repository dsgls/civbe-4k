...
["MapTypes"] = {
	FullSync = function()
		...
		pullDown:RegisterSelectionCallback( function(id)
			...
			-- Special case, if map script 0 is selected, tell the planet system a random
			-- planet is being selected.
			if ( id == 0 ) then
...
function OnDefaultsClicked()
	...
	-- Default Planet
	local planet = GameInfo.Planets["PLANET_DEFAULT"];
	if(planet == nil) then
		gameSpeed = GameInfo.Planets()();
	end
	PreGame.SetPlanet(planet.ID);
