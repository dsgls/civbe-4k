...
local defaultErrorTextureSheet = "BranchBacking_Unresearched.dds";

nullOffset = Vector2(0,0);
questionOffset, questionTextureSheet = IconLookup( 24, 64, "CIV_COLOR_ATLAS" );
...
function AdjustArtOnUniqueUnitButton( thisButton, thisFrame, thisUnitInfo, textureSize, extendedTooltip, noTooltip)
	if thisButton then
		...
		local textureOffset, textureSheet = IconLookup( thisUnitInfo.PortraitIndex, textureSize, thisUnitInfo.PortraitAtlas );				
...
function PopulateUniqueBonuses( controlTable, civ, _, extendedTooltip, noTooltip)
	...
	function PopulateButton(button, buttonFrame)
		...
		for row in DB.Query([[SELECT ID, Description, PortraitIndex, PortraitAtlas from Units
							JOIN ArtDefine_UnitInfos ON ArtDefine_UnitInfos.Type = UnitArtInfo
							INNER JOIN Civilization_UnitClassOverrides ON Units.Type = Civilization_UnitClassOverrides.UnitType 
							WHERE Civilization_UnitClassOverrides.CivilizationType = ?
							AND	Civilization_UnitClassOverrides.UnitType IS NOT NULL]], civ.Type) do
		...
		for row in DB.Query([[SELECT ID, Description, PortraitIndex, IconAtlas from Buildings
							INNER JOIN Civilization_BuildingClassOverrides ON Buildings.Type = Civilization_BuildingClassOverrides.BuildingType 
							WHERE Civilization_BuildingClassOverrides.CivilizationType = ?
							AND	Civilization_BuildingClassOverrides.BuildingType IS NOT NULL]], civ.Type) do
...
function PopulateUniqueBonuses_CreateCached()
	...
	local uniqueUnitsQuery = DB.CreateQuery([[SELECT ID, Description, PortraitIndex, PortraitAtlas from Units
								JOIN ArtDefine_UnitInfos ON ArtDefine_UnitInfos.Type = UnitArtInfo
								INNER JOIN Civilization_UnitClassOverrides ON Units.Type = Civilization_UnitClassOverrides.UnitType 
								WHERE Civilization_UnitClassOverrides.CivilizationType = ?
								AND	Civilization_UnitClassOverrides.UnitType IS NOT NULL]]);
								
	local uniqueBuildingsQuery = DB.CreateQuery([[SELECT ID, Description, PortraitIndex, IconAtlas from Buildings
								INNER JOIN Civilization_BuildingClassOverrides ON Buildings.Type = Civilization_BuildingClassOverrides.BuildingType 
								WHERE Civilization_BuildingClassOverrides.CivilizationType = ?
