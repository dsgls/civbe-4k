...
g_GraphVerticalMarkers = {
	...
	Controls.VerticalLabel6,
	Controls.VerticalLabel7
};
...
Panels = {

	{
		...
		CreateUpdateHandler = function(panel)
			...
			return function()
				if(not graphsPanel:IsHidden()) then
					...
					local CURSOR_ART_SIZE = 16;
					local xRelative = x - xAbsoluteOffset;
					local yRelative = (y - yAbsoluteOffset) - CURSOR_ART_SIZE;
		...
		DrawGraph = function(panel)
			...
			function DrawGraph(playerInfo, color, scoreType, graphWidth, minX, maxX, yScale, yOffset)
				...
				-- Offset draws in the Y direction per-civ to handle overlapping horizontal lines.
				local drawOffsetY = playerInfo["infoNum"];
				if ( drawOffsetY == nil ) then drawOffsetY = 0 end;
				...
				for i = 1, numSegments, 1 do
					...
					if(y1 ~= nil and y2 ~= nil) then
						...
						startY = (graphHeight - startY) - drawOffsetY;
						...
						-- inverse Y
						endY = (graphHeight - endY) - drawOffsetY;
			...
			if(minScore ~= nil) then	-- this usually means that there were no values for that dataset.
				...
				for i,v in ipairs(playerInfos) do
					...
					v["infoNum"]		= i;		-- Set player num, graphing may need this to offset lines, etc... to make it look better
			...
			function RefreshCivilizations()
				...
				for i, player in ipairs(g_ReplayInfo.PlayerInfo) do
					...
					if(player.LeaderName ~= "") then
						graphLegendInstance.LegendName:SetText(player.LeaderName);
					elseif(player.CivShortDescription ~= nil) then
					...
					print("Red: " .. color.Red .. ", Blue: " .. color.Blue .. ", Green: " .. color.Green .. ", Alpha: " .. tostring(color.Alpha));
...

	{
		...
		Refresh = function(panel)
			...
			-- ??TRON remove this, no need to pad!
