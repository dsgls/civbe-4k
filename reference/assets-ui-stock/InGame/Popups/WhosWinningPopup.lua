...
g_tListModeText[g_ListMode_Health] = "TXT_KEY_PROGRESS_SCREEN_HEALTH";
...
function IsListModeAllowed(iMode)
    ...
    -- Health restrictions
    elseif (iMode == g_ListMode_Health) then
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HEALTH)) then
			print("Progress Screen: Can't display Health because the system is off!");
			return false;
		end
...
function GetScoreFromMode(iPlayer, iMode)
	...
	-- Health Mode
	elseif (iMode == g_ListMode_Health) then
		return pPlayer:GetExcessHealth();
