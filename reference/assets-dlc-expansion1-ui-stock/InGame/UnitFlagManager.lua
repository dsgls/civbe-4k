...
local g_OrbitalManager = InstanceManager:new( "NewUnitFlag", "Anchor", Controls.OrbitalFlags );
...
local CityWorldPositionOffset = { x = 0, y = 0, z = 55 };
...
local g_UnitFlagClass = 
{
    ...
    new = function( self, playerID, unitID, fogState, invisible )
        ...
        then
			...
			if( pUnit:IsOrbitalUnit()) then
				o.m_InstanceManager = g_OrbitalManager;
		    elseif( pUnit:IsCombatUnit() and not pUnit:IsEmbarked()) then
    ...
    Initialize = function( o, playerID, unitID, fogState, invisible )
        ...
        local worldPosX, worldPosY, worldPosZ = GridToWorld( pUnit:GetX(), pUnit:GetY(), true );
    ...
    SetUnitColor = function( self )
		...
		if (pUnit ~= nil) then
			local iconColor, flagColor = pUnit:GetOwnerColors();
			...
			self.m_Instance.UnitIcon:SetColor( RGBAObjectToABGRHex(iconColor) );
			...
			self.m_Instance.Tier1:SetHide(true);
			self.m_Instance.Tier2:SetHide(true);
			self.m_Instance.Tier3:SetHide(true);

			local unitTier = Players[ self.m_PlayerID ]:GetBestUnitUpgradeTier(pUnit:GetUnitType());		
			if(not pUnit:IsAlienLifeform() and unitTier ~= nil and unitTier > 0)
			then
				self.m_Instance["Tier"..tostring(unitTier)]:SetColor( RGBAObjectToABGRHex(iconColor) );
				self.m_Instance["Tier"..tostring(unitTier)]:SetHide(false);
			end
		end
    end,
    ...
    SetUnitType = function( self )
		...
		self.m_HeightAdjust = flagHeightOffset;
    ...
    UpdateVisibility = function( self )
		local pUnit = Players[ self.m_PlayerID ]:GetUnitByID( self.m_UnitID );
		if (pUnit ~= nil or self.m_IsForceHide) then	-- Do this if forcing hidden, even if the unit is not there anymore.  We could be showing combat and since looking at Game Core object is looking into the future, the unit might already be gone.
    ...
    UpdateSelected = function( self, isSelected )
        ...
        if( self.m_IsSelected ) then
            ...
            if( self.m_IsOrbital ) then
				self.m_Instance.Anchor:ChangeParent( Controls.OrbitalFlags );
			elseif( self.m_IsAirCraft ) then
    ...
    UpdateFlagType = function ( self )
		...
		if (pUnit:HasInvisibility() and not pUnit:IsEmbarked()) then
			textureName = "UnitFlagInvisible.dds";
			maskName = "UnitFlagMask.dds";			
			-- Invisible or spotted now?
		...
		self.m_Instance.FlagBase:SetTextureOffsetVal( texOffsetX, texOffsetY );
        self.m_Instance.FlagBaseOutline:SetTexture( textureName );
        self.m_Instance.NormalSelect:SetTexture( selectedTextureName or textureName );
        self.m_Instance.HealthBarSelect:SetTexture( selectedTextureName or textureName );
    ...
    UpdateName = function ( self )
		...
		if(pUnit:HasName()) then
			local desc = Locale.ConvertTextKey("TXT_KEY_PLOTROLL_UNIT_DESCRIPTION_CIV",  pUnit:GetOwnerAdjectiveKey(), pUnit:GetNameKey());
			unitNameString = string.format("%s[NEWLINE](%s)", Locale.Lookup(pUnit:GetNameNoDesc()), desc);
		else
			unitNameString = Locale.ConvertTextKey("TXT_KEY_PLOTROLL_UNIT_DESCRIPTION_CIV",  pUnit:GetOwnerAdjectiveKey(), pUnit:GetNameKey());
		end

        if( active_team == team ) then
            local string;
            if(PreGame.IsMultiplayerGame() and self.m_Player:IsHuman()) then
                string = Locale.ConvertTextKey("TXT_KEY_MULTIPLAYER_UNIT_TT", self.m_Player:GetNickName(), pUnit:GetOwnerAdjectiveKey(), pUnit:GetNameKey() );
			else
            ...
            if(PreGame.IsMultiplayerGame() and self.m_Player:IsHuman()) then
				self.m_Instance.UnitIcon:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_MULTIPLAYER_UNIT_TT", self.m_Player:GetNickName(), pUnit:GetOwnerAdjectiveKey(), pUnit:GetNameKey()));
			else
    ...
    UpdateCargo = function( self )
        ...
        if( not pUnit:HasCargo() ) then
			...
			local textWidth = controlTable.Button:GetTextControl():GetSizeX();
			local buttonHeight = controlTable.Button:GetSizeY();
			local newWidth = math.max(150, textWidth+50);
			controlTable.Button:SetSizeVal(newWidth, buttonHeight);
			controlTable.ButtonHL:SetSizeVal(newWidth, buttonHeight);
			controlTable.Button:SetText( pUnit:GetName() );
            ...
            for i = 0, numUnits - 1 do
                ...
                if( pPlotUnit:IsCargo() and pPlotUnit:GetTransportUnit():GetID() == self.m_UnitID ) then
					...
					controlTable.ButtonHL:SetSizeVal(newWidth,bh);
					controlTable.Button:SetText(unitName);
					...
					local pCargoFlag = nil;
					if g_MasterList[ pPlotUnit:GetOwner() ] ~= nil then
						pCargoFlag = g_MasterList[ pPlotUnit:GetOwner() ][ pPlotUnit:GetID() ];
					end
    				-- Check to see if it is there, it is possible it has yet to be created
    ...
    -----------------------------------------------------------------
    ------------------------------------------------------------------
    HidePulldown = function( self )
		if( self.m_CargoControls ~= nil) then
			self.m_CargoControls.PullDown:ForceClose();
		end
	end,
	...
	UpdateFlagOffset = function( self )
        ...
        local offset = Vector2( 0, self.m_HeightAdjust );
		
		if (pUnit:IsInOrbit()) then
			offset = InOrbitOffset;
		else     
			...
			elseif plot:IsCity() or Game.HasEarthlingSettlement(plot) then
...
-- On Unit Created
-------------------------------------------------
function OnUnitCreated( playerID, unitID, hexVec, unitType, cultureType, civID, primaryColor, secondaryColor, fogState, selected, military, notInvisible, embarkChange )
...
function UpdateCityCargo( pPlot )
    ...
    if( cargoCount > 0 and pPlot:IsCity() ) then
		...
		-- set the hidden flag based on visibility of the plot
		local visible : boolean = pPlot:IsVisible(Game.GetActiveTeam(), false);
        cityFlagInstance.PullDown:SetHide(not visible);
...
function OnEnterCityScreen()
    ...
    for playerID,playerTable in pairs( g_MasterList ) do
        for unitID, pUnitFlag in pairs( playerTable ) do
			if (pUnitFlag ~= nil) then
				pUnitFlag:HidePulldown();
			end
        end
    end

	for cityIndex, pCityFlag in pairs(g_CityFlags) do
		pCityFlag.PullDown:ForceClose();
	end
end
...
-------------------------------------------------
-- On Unit Visibility Changed
-------------------------------------------------
function OnUnitVisibility( playerID, unitID, visible, changeInvisibility, blendTime )
	if changeInvisibility then
...
------------------------------------------------------------
-- Unit flag colors needs to change (e.g. alien units when their aggression level rises
------------------------------------------------------------
function OnPlayerColorChanged(playerID)

	if Players[ playerID ] == nil then
        return;
    end
    
	for unitID, pFlag in pairs( g_MasterList[playerID] ) do
		pFlag:SetUnitColor();
	end
end
Events.PlayerColorChanged.Add( OnPlayerColorChanged );
            ...
            for unit in player:Units() do
                ...
                if( plot ~= nil ) then
                    if( plot:IsVisible( Game.GetActiveTeam() ) ) then
                        OnUnitCreated( player:GetID(), unit:GetID(), 0, 0, 0, 0, 0, 0, 0, WhiteFog, 0, 0, true, false );
                    end
                end
            end
        end
			...
			do
				...
				if ( pUnit ~= nil ) then        	
					...
					if (PreGame.IsMultiplayerGame()) then
						local nickName		= TruncateStringByLength(pPlayer:GetNickName(), 20);
						toolTipString = Locale.ConvertTextKey("TXT_KEY_MULTIPLAYER_UNIT_TT", nickName, pPlayer:GetCivilizationAdjectiveKey(), pUnit:GetNameKey());
					else
						if (pUnit:HasName()) then
