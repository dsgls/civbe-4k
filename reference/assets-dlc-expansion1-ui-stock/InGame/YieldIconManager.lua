...
function BuildYield( x, y, index )
	...
	--local LocalWorldPos = GridToWorld( x, y );
	local LocalHexPos = ToHexFromGrid( Vector2( x, y ) );
	local LocalWorldPos = HexToWorldClamped( LocalHexPos );	-- take into account water height
    anchor.Anchor:SetWorldPosition( LocalWorldPos );
end
...
----------------------------------------------------------------
-- Fungal Marvel effect has changed the yields of a plot
----------------------------------------------------------------
function OnPlotAffectedByFungalEffect(plotIndex : number)
	
	-- reset yield data for plot
	g_YieldInfoCache[plotIndex] = nil;
end
GameEvents.PlotAffectedByFungalEffect.Add(OnPlotAffectedByFungalEffect);
