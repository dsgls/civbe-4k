...
local FLASHING_VIRTUE		= 5;
local FLASHING_NEEDS_ORDERS	= 6;
local FLASHING_PLEASE_WAIT	= 7;
...
function SetEndTurnFlashing (iFlashingState)
	...
	local bHideVirtueFlash     = true;
	local bHideUnitNeedsOrders = true;
	local bHidePleaseWait	   = true;
	...
	elseif (iFlashingState == FLASHING_VIRTUE) then
		bHideVirtueFlash = false;
	elseif (iFlashingState == FLASHING_NEEDS_ORDERS) then
		bHideUnitNeedsOrders = false;
	elseif (iFlashingState == FLASHING_PLEASE_WAIT) then
		bHidePleaseWait = false;
	...
	Controls.EndTurnButtonVirtueAlpha:SetHide(bHideVirtueFlash);
	Controls.EndTurnButtonNextTurnAlpha:SetHide( bHideEndTurnFlash );
	Controls.EndTurnButtonPleaseWaitAlpha:SetHide( bHidePleaseWait );

	Controls.PleaseWait_Round:SetHide( bHidePleaseWait );
	Controls.UnitNeedsOrders_Round:SetHide( bHideUnitNeedsOrders );
	Controls.EndTurnButton_Round:SetHide( bHideEndTurnFlash );
...
function OnEndTurnClicked()
	...
	if (   blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_POLICY		
		...
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_QUEST_PROMPT
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_UNIT_UPGRADE_PROMPT
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_FREE_AFFINITY_LEVEL
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_PLANETFALL
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_SELECT_NATIONAL_SECURITY_PROJECT
		or blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_DIPLOMACY_PENDING_TRANSACTIONS) 
	then
		UI.ActivateNotification(blockingNotificationIndex);
	elseif ( blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_RESEARCH ) then
	...
	elseif (blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_STACKED_UNITS
		...
		HandleEndTurnUnitBlockerClick(blockingType);	
...
Controls.PleaseWait_Round:RegisterCallback( Mouse.eLClick, OnEndTurnClicked );
Controls.EndTurnButton_Round:RegisterCallback( Mouse.eLClick, OnEndTurnClicked );
Controls.UnitNeedsOrders_Round:RegisterCallback( Mouse.eLClick, OnEndTurnClicked );
...
function setEndTurnWaiting()
		...
		if (iPlayerLoop ~= iActivePlayer) then
			...
			if(otherPlayer ~= nil) then
				if(otherPlayer:IsHuman() and not otherPlayer:HasReceivedNetTurnComplete()) then					
					...
					local nickName		= TruncateStringByLength(otherPlayer:GetNickName(), MAX_NAME_CHARS);
					local playerName	= otherPlayer:GetName();					
					pleaseWait = pleaseWait .. "[NEWLINE]" .. "(" .. nickName .. ") " .. playerName;
	...
	Controls.AdditionalToolTipHitArea:SetToolTipString( pleaseWait );

	SetEndTurnFlashing(FLASHING_PLEASE_WAIT);
end

function HandleEndTurnUnitBlockerClick(blockingType)
	-- Select first ready unit.
	local player = Players[Game.GetActivePlayer()];
	local pUnit = player:GetFirstReadyUnit();
	if(DoEndTurnUnitClick(pUnit)) then
		-- Selected first ready unit, we're good.
		return true;
		
	-- If this is a ENDTURN_BLOCKING_UNIT_NEEDS_ORDERS blocker,
	-- this could be due to an autoendturn player having manually selected a unit
	-- and blocked the autoendturn.  In that case, cycle the unit selection to clear the block.
	elseif(blockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_UNIT_NEEDS_ORDERS) then
		Game.CycleUnits(true, true, false);
	else
		print("LOCK! Due to blocking type "..blockingType);
		--??TRON: Some function (this isn't exposed) needs to reset blocking type: player:SetEndTurnBlocking( EndTurnBlockingTypes.NO_ENDTURN_BLOCKING_TYPE );
	end
	
	return false;
end

function DoEndTurnUnitClick(pUnit)
	if (pUnit ~= nil) then
...
function OnEndTurnDirty()
	...
	if (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_POLICY) then
		...
		iFlashingState = FLASHING_VIRTUE;
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_UNITS) then
		...
		buttonDisabled = false;
		iFlashingState = FLASHING_NEEDS_ORDERS;
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_UNIT_NEEDS_ORDERS) then
		-- This block type is for when an autoendturn user has manually selected a unit to stop 
		-- auto end turns.  Just show the Next Turn button graphics in that case.
		strEndTurnMessage = nextTurnString;
		strButtonToolTip = nextTurnTooltipString;
		buttonDisabled = false;		
		iFlashingState = FLASHING_END_TURN;	
		Controls.UnitNeedsOrders_Round:SetHide( false );			
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_FREE_POLICY) then
		...
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_FREE_VIRTUE_SUMMARY");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_FREE_VIRTUE");
		buttonDisabled = false;
		iFlashingState = FLASHING_VIRTUE;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_FOUND_PANTHEON) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_SUMMARY_BELIEF_AVAILABLE");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_BELIEF_AVAILABLE");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;	
	...
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_CHOOSE_IDEOLOGY) then
		...
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_QUEST_PROMPT) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_QUEST_ACTION_REQUIRED");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_QUEST_ACTION_REQUIRED_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_UNIT_UPGRADE_PROMPT) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_UNIT_UPGRADES_AVAILABLE");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_UNIT_UPGRADES_AVAILABLE_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_FREE_AFFINITY_LEVEL) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_FREE_AFFINITY_LEVEL");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_FREE_AFFINITY_LEVEL_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_PLANETFALL) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_PLANETFALL");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_PLANETFALL_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_SELECT_NATIONAL_SECURITY_PROJECT) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_SELECT_NATIONAL_SECURITY_PROJECT");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_SELECT_NATIONAL_SECURITY_PROJECT_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	elseif (EndTurnBlockingType == EndTurnBlockingTypes.ENDTURN_BLOCKING_DIPLOMACY_PENDING_TRANSACTIONS) then
		strEndTurnMessage = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_DIPLOMACY_PENDING_TRANSACTIONS");
		strButtonToolTip = Locale.ConvertTextKey("TXT_KEY_NOTIFICATION_DIPLOMACY_PENDING_TRANSACTIONS_TT");
		buttonDisabled = false;
		iFlashingState = FLASHING_END_TURN;
	else
	...
	Controls.AdditionalToolTipHitArea:SetToolTipString( strButtonToolTip );
