...
function ProcessInput( uiMsg, wParam, lParam )
    if( uiMsg == MouseEvents.MouseMove or uiMsg == MouseEvents.PointerUpdate ) then
	...
	-- Don't eat up input, or rest of UI will never get a crack at it!
	return false;	
end
...
function Reset()
	...
	if( m_iTipLevel ~= 0 ) then
		if(OptionsManager.GetTooltip1Seconds() > 0) then
...
function OnUpdate( fDTime )
    ...
    if bHasMouseOver then
		if m_pPlotHelpText:IsHidden() then
			m_pPlotHelpText:SetHide( false );
		end
    else
		if not m_pPlotHelpText:IsHidden() then
    ...
    if( m_iTipLevel == 0 ) then
		...
		-- Ignore time if touch screen is enabled, we want instant updates
        if( m_fTime > (OptionsManager.GetTooltip1Seconds() / 100) or UI.IsTouchScreenEnabled() ) then
			Level1Tip();			
            m_iTipLevel = 1;
        end
...
function Level1Tip()
	...
	if (plot and plot:IsRevealed(iActiveTeam, bIsDebug)) then
		...
		-- under construction display (if owner or neutral)
		if (plot:GetTeam() == iActiveTeam) then
		...
		if plot:GetOwner() == Game.GetActivePlayer() then
			local iEnergyMaintenance = plot:GetPlotMaintenance(Game.GetActivePlayer());
			if iEnergyMaintenance > 0 then

				local strMaintenance = Locale.ConvertTextKey("[ICON_ENERGY] " .. iEnergyMaintenance);
				...
				TextString = TextString .. "[COLOR_NEGATIVE_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_CITYVIEW_MAINTENANCE") .. "[ENDCOLOR]" .. " : " .. strMaintenance;
			end
		end
