...
local g_GameOptionIM = InstanceManager:new( "GameOption", "Text", Controls.GameOptions );
local g_GameOptionsList = {};

local g_GameSetupIM = InstanceManager:new( "GameSetup", "Root", Controls.SetupOptions );
local g_GameSetupList = {};

local g_VictoryConditionsIM = InstanceManager:new( "GameOption", "Text", Controls.VictoryConditions );
local g_VictoryConditionsList = {};

local g_AdvancedOptionIM = InstanceManager:new( "TipGameOption", "Text", Controls.AdvancedOptions );
...
function OnEnter()
	if (not g_bEntered) then
		-- Moving between sub-menus causes this screen to be shown and Hide
...
-- ===========================================================================
function OnStarshipsButtonPressed()
	UI.SendSaveDataToPartnerApp();
end
Controls.StarshipsButton:RegisterCallback(Mouse.eLClick, OnStarshipsButtonPressed);
...
function OnMainMenu()
	...
	RaiseConfirmDialog( yesCallback, nil, "TXT_KEY_MENU_RETURN_MM_WARN" );
end
...
function OnRestartGame()
	...
	RaiseConfirmDialog( yesCallback, nil, "TXT_KEY_MENU_EXIT_DEFAULT" );
end
...
function OnShowHide( isHide, isInit )
	...
	-- Certain popups, if open, must be dealt with before a game can be saved or loaded
	if (UI.IsPopupTypeOpen(ButtonPopupTypes.BUTTONPOPUP_CITY_CAPTURED)) then
		bHideLoadGameOption = true;
		bHideSaveGameOption = true;
		bDisableQuickSave = true;
	end
	...
	Controls.StarshipsButton:SetDisabled( not FiraxisLive.IsConnected() );
	...
	LuaEvents.IgnoreTutorialUpdate( not isHide );
end
...
function PopulateGameData()
	...
	CivIconHookup( iPlayer, 64, Controls.CivIcon, Controls.CivIconBG, nil, false, false, Controls.CivIconHighlight );
	...
	if(Game:IsNetworkMultiPlayer() and pPlayer:GetNickName() ~= "") then
		...
		Controls.CivIconFrame:SetToolTipString( Locale.ConvertTextKey( myCivInfo.ShortDescription ));
	...
	elseif( iPlayer == Game.GetActivePlayer() and PreGame.GetCivilizationShortDescription( 0 ) ~= "" ) then
		Controls.CivIconFrame:SetToolTipString( PreGame.GetCivilizationShortDescription( 0 ));
	...
	local planetInfo = GameInfo.Planets[PreGame.GetPlanet()];
	Controls.TypeLabel:SetString(MapUtilities.GetPlanetTypeDescription(mapInfo.Name, planetInfo.ID));
	if (planetInfo.ToolTip ~= nil) then
		local tipString : string = "";

		if( mapInfo.Description ~= nil ) then
			tipString = tipString .. Locale.Lookup(mapInfo.Description) .. "[NEWLINE][NEWLINE]";
		end

		tipString = tipString .. Locale.Lookup(planetInfo.ToolTip);

		for row in GameInfo.PlanetEffects{ PlanetType = planetInfo.Type } do
			if (row.ToolTip ~= nil) then
				tipString = tipString .. "[NEWLINE][NEWLINE]" .. Locale.Lookup(row.ToolTip);
			end
		end
		Controls.TypeLabel:SetToolTipString(tipString);
	end
	...
	if ( info ~= nil ) then
		...
		Controls.SizeLabel:SetToolTipString( Locale.ConvertTextKey( info.Help ) );
	else
    ...
    if ( info ~= nil ) then
		...
		Controls.DifficultyLabel:SetToolTipString( Locale.ConvertTextKey( info.Help ) );
    else
    ...
    if ( info ~= nil ) then
		...
		Controls.SpeedLabel:SetToolTipString( Locale.ConvertTextKey( info.Help )  );
    else
...
function PopulateGameOptions()
	...
	local info = GameInfo.Colonists[PreGame.GetLoadoutColonist(iPlayer)];
	if (info ~= nil) then
		local controlTable = g_GameSetupIM:GetInstance();
		g_GameSetupList[count] = controlTable;
		controlTable.TextLabel:LocalizeAndSetText("TXT_KEY_CHOOSE_LOADOUT_COLONISTS");
		controlTable.Text:LocalizeAndSetText(info.ShortDescription);
		controlTable.Text:SetToolTipString(Locale.ConvertTextKey(info.Description));
		count = count + 1;
	end

	local info = GameInfo.Cargo[PreGame.GetLoadoutCargo(iPlayer)];
	if (info ~= nil) then
		local controlTable = g_GameSetupIM:GetInstance();
		g_GameSetupList[count] = controlTable;
		controlTable.TextLabel:LocalizeAndSetText("TXT_KEY_CHOOSE_LOADOUT_CARGO");
		controlTable.Text:LocalizeAndSetText(info.ShortDescription);
		controlTable.Text:SetToolTipString(Locale.ConvertTextKey(info.Description));
		count = count + 1;
	end

	local info = GameInfo.Spacecraft[PreGame.GetLoadoutSpacecraft(iPlayer)];
	if (info ~= nil) then
		local controlTable = g_GameSetupIM:GetInstance();
		g_GameSetupList[count] = controlTable;
		controlTable.TextLabel:LocalizeAndSetText("TXT_KEY_CHOOSE_LOADOUT_SPACECRAFT");
		controlTable.Text:LocalizeAndSetText(info.ShortDescription);
		controlTable.Text:SetToolTipString(Locale.ConvertTextKey(info.Description));
		count = count + 1;
	end

	-- Set Max turns if necessary
	local maxTurns = PreGame.GetInitialMaxTurns();
	if ( maxTurns ~= 0 ) then
		local controlTable = g_GameOptionIM:GetInstance();
		g_GameOptionsList[count] = controlTable;
		controlTable.Text:LocalizeAndSetText("TXT_KEY_AD_SETUP_MAX_TURNS_1", maxTurns);
		count = count + 1;
	end
	...
	Controls.GameOptions:CalculateSize();
	Controls.GameOptions:ReprocessAnchoring();
...
-- ===========================================================================
function LowerConfirmDialog()
	Controls.ExitConfirm:SetHide( true );
end


-- ===========================================================================
--	Raise a Yes/No blocking dialog
--	yesCallback,	function to execute when "YES" is pressed.
--	noCallback,		function to execute when "NO" is pressed (or nil to just dismiss)
--	optionalMessage	default used if non-present
-- ===========================================================================
function RaiseConfirmDialog( yesCallback, noCallback, optionalMessage )

	local message = "TXT_KEY_MENU_EXIT_DEFAULT";
	if ( optionalMessage ~= nil and optionalMessage ~= "" ) then
		message = optionalMessage;
	end

	-- Change any NIL parameters to empty functions.
	...
	Controls.Message:SetText( Locale.ConvertTextKey(message) );
	Controls.ExitConfirm:SetHide( false );

	Controls.Yes:RegisterCallback( Mouse.eLClick,	function() LowerConfirmDialog(); yesCallback(); end );
	Controls.No:RegisterCallback( Mouse.eLClick,	function() LowerConfirmDialog(); noCallback(); end );
end
