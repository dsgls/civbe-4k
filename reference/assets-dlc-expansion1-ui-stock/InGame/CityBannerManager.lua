...
function ShouldShowOutpostRangeStrikeButton(outpost) 
	if outpost == nil then
		return false;
	end
	
	if (outpost:GetOwner() ~= Game.GetActivePlayer()) then
		return false;
	end
		
	local outpostSite = outpost:GetStrategicSite();
	return outpostSite and outpostSite:CanRangeStrikeNow();
end
...
function UpdateOutpostRangeIcons(outpostBanner)
	...
	if (player ~= nil) then
		...
		if (controls.CityAttackFrame ~= nil) then
			if (ShouldShowOutpostRangeStrikeButton(outpost)) then
				if(controls.IconsStack:GetSizeX() ~= 3) then
					controls.CityAttackFrame:SetOffsetY(105);
					controls.CityAttackFrame:SetColor(0,0,0,0);
				else
				...
				local isHidingRangeStrike = not ShouldShowOutpostRangeStrikeButton(outpost);
				controls.CityRangeStrikeContainer:SetHide( isHidingRangeStrike );
			else
				controls.CityAttackFrame:SetHide(true);
			end
...
--[[
	...
	if city ~= nil then
		...
		if (UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION) then
			...
			local hasMet : boolean = Teams[Game.GetActiveTeam()]:IsHasMet(player:GetTeam()) or Players[Game.GetActivePlayer()]:IsObserver();
		...
		-- Update strength
		local cityStrengthStr = city:GetStrengthValue() / 100;
		...
		controls.CityStrength			:SetText( Locale.ToNumber( cityStrengthStr , "#.#" ) );				
		...
		-- Show the city banner for a player's capital if we've met them or if we're an observer
		local hasMet : boolean = Teams[player:GetTeam()]:IsHasMet(Players[Game.GetActivePlayer()]:GetTeam());
		local isObserver : boolean = Players[Game.GetActivePlayer()]:IsObserver();
		if (isCapital and (hasMet or isObserver)) then
			controls.Anchor:SetHide(false);
		end
	end
...
function RefreshOutpostDamage(outpost, outpostBanner, damage)
	local controls = outpostBanner.SubControls;
	local strength = outpost:GetCombatStrength() / 100;

	local maxHP			= outpost:GetMaxHitPoints();
	local curHP			= maxHP - damage;
	local defenseStr	= Locale.ConvertTextKey("TXT_KEY_NUM_COMBAT_STRENGTH", Locale.ToNumber(strength, "#.#"));
...
--	controls.CityName:SetColor(textColorSoft, 2);
	...
	if outpost ~= nil then
		...
		-- Update the strike icons
		if outpostBanner.SubControls.CityAttackStack ~= nil then
			UpdateOutpostRangeIcons(outpostBanner);
			outpostBanner.SubControls.CityAttackStack:CalculateSize();
			outpostBanner.SubControls.CityAttackStack:ReprocessAnchoring();
		end
...
function OnCityCreated( hexPos, playerID, cityID, cultureType, eraType, continent, size, fowState )
	...
	local hasMetActivePlayer : boolean = Teams[team]:IsHasMet(Players[Game.GetActivePlayer()]:GetTeam()) or Players[Game.GetActivePlayer()]:IsObserver();
	...
	if fowState == BlackFog or not hasMetActivePlayer then
...
function OnOutpostCreated(hexPos, playerID, outpostID, fowState)
	...
	if(iActiveTeam ~= team) then
	    ...
	    controlTable.BannerButton:SetVoid1( gridPosX );
	    controlTable.BannerButton:SetVoid2( gridPosY );
		controlTable.CityRangeStrikeButton:RegisterCallback( Mouse.eLClick, OnOutpostRangeStrikeButtonClick );		
		controlTable.CityRangeStrikeButton:SetVoid1(playerID);
		controlTable.CityRangeStrikeButton:SetVoid2(outpostID);
...
function RefreshCityDamage(city, cityBanner, damage)
	local controls = cityBanner.SubControls;
	local strength = city:GetStrengthValue() / 100;
...
function OnHexFogEvent( hexPos, fowType, bWholeMap )
    if( bWholeMap ) then
		...
		if plot ~= nil then
			-- Check plot for city
			local city : object = plot:GetPlotCity();
			local outpost : object = plot:GetPlotOutpost();
			local station : object = Game.GetStationByID(plot:GetPlotIndex());
			...
			if (city ~= nil) then
				...
				if player ~= -1 then
					...
					if playerTable then
						...
						if instance then
							local hasMet : boolean = Teams[Players[player]:GetTeam()]:IsHasMet(Players[Game.GetActivePlayer()]:GetTeam()) or Players[Game.GetActivePlayer()]:IsObserver();

							if fowType == BlackFog or not hasMet then
								instance.SubControls.Anchor:SetHide( true );
			...
			elseif (outpost ~= nil) then
				local outpostID : number = outpost:GetID();
				local player : number = outpost:GetOwner();
				if (player ~= -1) then
					local playerTable : table = OutpostInstances[player];
					if (playerTable ~= nil) then
						local instance : table = playerTable[outpostID];
						if (instance ~= nil) then
							local hasMet : boolean = Teams[Players[player]:GetTeam()]:IsHasMet(Players[Game.GetActivePlayer()]:GetTeam()) or Players[Game.GetActivePlayer()]:IsObserver();
...
-------------------------------------------------
-- On Outpost Range Strike Button Selected
-------------------------------------------------
function OnOutpostRangeStrikeButtonClick( PlayerID, OutpostID )
	local player = Players[PlayerID];
	if (player == nil) then
		--print("Invalid player");
		return;
	...
	local outpost = player:GetOutpostByID(OutpostID);
	...
	local bAlt = false;
	local bShift = false;
	local bIgnoreUnits = true;

	UI.LocationSelect(outpost:GetPlot(), bCtrl, bAlt, bShift, bIgnoreUnits);

	--Events.InitCityRangeStrike( PlayerID, OutpostID );
end
...
function LeaderSelected( ePlayer )
	...
	if (Game.GetNumPendingTransactions(ePlayer, Game.GetActivePlayer()) > 0) then
		local transactions : table = Game.GetPendingTransactions(ePlayer, Game.GetActivePlayer());

		if (#transactions > 0) then
			Events.SerialEventGameMessagePopup{Type = ButtonPopupTypes.BUTTONPOPUP_DIPLOMACY_PENDING_TRANSACTIONS, Data2 = ePlayer, Data3 = transactions[1]:GetID() };
		end
	else
		Events.SerialEventGameMessagePopup{Type = ButtonPopupTypes.BUTTONPOPUP_DIPLOMACY_OVERVIEW, Data2 = ePlayer};
	end
...
function OnBannerClick( x, y )
	...
	if plot then
		...
		elseif (Teams[Game.GetActiveTeam()]:IsHasMet(player:GetTeam()) or Players[Game.GetActivePlayer()]:IsObserver()) then
...
function OnOutpostBannerClick (x, y)
	...
	elseif (player ~= nil) then
		...
		if ( Teams[activeTeam]:IsHasMet( playerTeam ) or Players[Game.GetActivePlayer()]:IsObserver()) then
			if player:IsMinorCiv() then
				UI.DoSelectCityAtPlot( plot );
			else
				LeaderSelected( playerID );
