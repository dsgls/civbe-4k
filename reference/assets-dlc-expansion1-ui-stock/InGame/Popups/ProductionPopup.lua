...
include("TabSupport.lua");

local g_UnitInstanceManager : table		= InstanceManager:new( "ProdButton", "Top", Controls.UnitButtonStack );
local g_BuildingInstanceManager : table = InstanceManager:new( "ProdButton", "Top", Controls.BuildingButtonStack );
local g_WonderInstanceManager : table	= InstanceManager:new( "ProdButton", "Top", Controls.WonderButtonStack );
local g_ProcessInstanceManager : table	= InstanceManager:new( "ProdButton", "Top", Controls.OtherButtonStack );
local g_PlotSelectButtonIM : table		= nil;

local m_PopupInfo : table = nil;

local bHidden : boolean = true;
local m_cachedCompletedEntries : table = {};

local m_purchaseTabs : table = {};
local PurchaseMode : table = 
{
	ENERGY = 1,
	CAPITAL = 2,
}
local m_purchaseMode = PurchaseMode.ENERGY;

------------------------------------------------
-- global constants
------------------------------------------------
g_strInfiniteTurns = Locale.ConvertTextKey("TXT_KEY_PRODUCTION_HELP_INFINITE_TURNS");
g_moveCityProjecInfo = GameInfo.Projects["PROJECT_MOVE_CITY"];
g_moveCityPediaArticle = GameInfo.Concepts["CONCEPT_MOVING_WATER_CITIES"];
local g_MoveCityValidStyle : string		= "MoveCityValid";
local g_MoveCityInvalidStyle : string	= "MoveCityInvalid";
local g_MoveCityHoverStyle : string		= "MoveCityHover";
...
-- Defines used to track building/unit and energy/capital information on city purchase menu with a single int
local g_PURCHASE_UNIT_ENERGY = 0;
local g_PURCHASE_BUILDING_ENERGY = 1;
local g_PURCHASE_PROJECT_ENERGY = 2;
local g_PURCHASE_UNIT_CAPITAL = 3;
local g_PURCHASE_BUILDING_CAPITAL = 4;
local g_PURCHASE_PROJECT_CAPITAL = 5;
local g_CONSTRUCT_UNIT = 6;
local g_CONSTRUCT_BUILDING = 7;
local g_CONSTRUCT_PROJECT = 8;
local g_MAINTAIN_PROCESS = 9;
local COLOR_WARNING = 0xC23D3090;
local COLOR_WHITE =   0xffB7ADA4; --This is not actually white, but Color0 of the style ProductionListText.
...
function ProductionSelected( ePurchaseEnum, iData)
	...
	elseif (ePurchaseEnum == g_PURCHASE_PROJECT_ENERGY) then
	   ...
	   eYield = YieldTypes.YIELD_ENERGY;
	elseif (ePurchaseEnum == g_PURCHASE_UNIT_CAPITAL) then
	   eOrder = OrderTypes.ORDER_TRAIN;
	   eYield = YieldTypes.YIELD_CAPITAL;
	elseif (ePurchaseEnum == g_PURCHASE_BUILDING_CAPITAL) then
	   eOrder = OrderTypes.ORDER_CONSTRUCT;
	   eYield = YieldTypes.YIELD_CAPITAL;
	elseif (ePurchaseEnum == g_PURCHASE_PROJECT_CAPITAL) then
	   eOrder = OrderTypes.ORDER_CREATE;
	   eYield = YieldTypes.YIELD_CAPITAL;
	...
	if eOrder == OrderTypes.ORDER_CREATE then
		...
		if projectInfo ~= nil then
			if projectInfo.PlotProject then
				-- Move City plot selection
				if (projectInfo.ID == g_moveCityProjecInfo.ID) then
					DoMoveCityPlotSelection (eOrder, iData, projectInfo);
				-- Wonder plot selection
				else
		...
		-- Restore selection interface mode if we were presently in plot-select mode and are clicking on a different production button
		if (UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_WONDER_PLOT_SELECTION or 
			UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_MOVE_CITY)
		then
    ...
    if g_IsProductionMode then
		...
		if (eOrder == OrderTypes.ORDER_TRAIN or eOrder == OrderTypes.ORDER_CONSTRUCT or eOrder == OrderTypes.ORDER_CREATE) then
			...
			elseif (eYield == YieldTypes.YIELD_FAITH) then
				Events.AudioPlay2DSound("AS2D_INTERFACE_CITY_SCREEN_PURCHASE");
			elseif (eYield == YieldTypes.YIELD_CAPITAL) then
				Events.AudioPlay2DSound("AS2D_INTERFACE_CITY_SCREEN_PURCHASE");
			end
		end
    end
...
function OnMoveCityOptionSelected()
	g_IsPlotChoiceProject = true;

	local eOrder : number = OrderTypes.ORDER_CREATE;
	local iData : number = g_moveCityProjecInfo.ID;

	DoMoveCityPlotSelection (eOrder, iData, g_moveCityProjecInfo);
end
...
function UpdateWindow( city, focusOnCity )
	...
	if (UI.GetInterfaceMode() ~= InterfaceModeTypes.INTERFACEMODE_MOVE_CITY) then
		ClearMoveCityHighlights();
	end
	...
	Controls.PurchaseSelectorTabs:SetHide(g_IsProductionMode);
	...
	g_activePerkTypes = player:GetAllActivePlayerPerkTypes();
	...
	-- Unit Sort Function
	--	Sort in order of Civilians to Military
	--	Prefer military that matches the city's domain (land or sea)
	function GetUnitSortPriority(city : object, unitInfo : object)
		-- sort flags
		local civilian : boolean = unitInfo.CivilianAttackPriority ~= nil;
		local land : boolean = unitInfo.Domain == "DOMAIN_LAND" or unitInfo.Domain == "DOMAIN_HOVER";
		local sea : boolean = unitInfo.Domain == "DOMAIN_SEA" or unitInfo.Domain == "DOMAIN_AMPHIBIOUS";
		local aircraft : boolean = unitInfo.Domain == "DOMAIN_AIR";

		-- civilians always at the top
		if (civilian == true) then
			return 0;
		end
		-- military units that match domain (land or sea)
		if (city ~= nil) then
			if (land == true and not city:IsWater()) then
				return 1;
			elseif (sea == true and city:IsWater()) then
				return 1;
			end
		end
		-- aircraft
		...
		-- everything else:
		-- military units that DON'T match domain (land or sea)
		return 3;
	end
	...
	local purchaseYieldType : number = YieldTypes.NO_YIELD;
	if (m_purchaseMode == PurchaseMode.ENERGY) then
		purchaseYieldType = YieldTypes.YIELD_ENERGY
	elseif (m_purchaseMode == PurchaseMode.CAPITAL) then
		purchaseYieldType = YieldTypes.YIELD_CAPITAL;
	else
		error("Unhandled purchase mode");
	end
 		...
 		if g_IsProductionMode then
			...
			if city:CanTrain( unitID, false, true ) then
				...
				table.insert(listUnits, {
					...
					SortPriority = GetUnitSortPriority(city, unit)
				});
			end
 		else
			if city:IsCanPurchase(false, false, unitID, -1, -1, purchaseYieldType) then
 				local isDisabled = false;
     			-- test w/o visible (ie can train right now)
	     	
    			if (not city:IsCanPurchase(true, true, unitID, -1, -1, purchaseYieldType)) then
    				isDisabled = true;
				end
	 			
 				local cost = city:GetUnitPurchaseCost( unitID, purchaseYieldType );
 	...
 	for project in GameInfo.Projects() do
 		local projectID = project.ID;
 	 	if g_IsProductionMode then
			...
			if city:IsCanPurchase(false, false, -1, -1, projectID, purchaseYieldType) then
				local isDisabled = true;		    
 				local cost = city:GetProjectPurchaseCost( projectID, purchaseYieldType );
    ...
    -- Buildings	
 	for building in GameInfo.Buildings() do
 		...
 		if g_IsProductionMode then
			...
			if city:IsCanPurchase(false, false, -1, buildingID, -1, purchaseYieldType) then
				local col = 2;
				local thisBuildingClass = GameInfo.BuildingClasses[building.BuildingClass];
				if thisBuildingClass.MaxGlobalInstances > 0 or thisBuildingClass.MaxPlayerInstances == 1 or thisBuildingClass.MaxTeamInstances > 0 then
					col = 3;
 				...
 				local isDisabled = false;
    			if (not city:IsCanPurchase(true, true, -1, buildingID, -1, purchaseYieldType)) then
 					isDisabled = true;
 				end
	 			
 				local cost = city:GetBuildingPurchaseCost( buildingID, purchaseYieldType );
				local building = {
					ID = buildingID,
					Description = building.Description,
					DisplayDescription = Locale.Lookup(building.Description),
	...
	local GenericSort = function(a,b)
		...
		if(aSort == bSort) then
			...
			if(comp == 0) then
				return a.YieldType ~= YieldTypes.YIELD_CAPITAL;
			else
				return comp == -1;
			end
		else
	...
	-- Move City control
	if (city:IsWater() and g_IsProductionMode) then
		Controls.MoveCityGrid:SetHide(false);

		local moveHelpStr : string = Locale.Lookup(g_moveCityProjecInfo.Help);
		
		local cityTeam : object = Teams[city:GetTeam()];
		if (cityTeam and not cityTeam:HasDeepWaterCityMovement()) then
			moveHelpStr = moveHelpStr.."[NEWLINE][NEWLINE]"..Locale.Lookup("TXT_KEY_PROJECT_MOVE_CITY_SHALLOW_WARNING");
		end
		Controls.MoveCityButton:SetToolTipString(moveHelpStr);

		if (bGeneratingProduction) then
			strTurnsLeft = Locale.ConvertTextKey("TXT_KEY_STR_TURNS", city:GetProjectProductionTurnsLeft(g_moveCityProjecInfo.ID));
		else
			strTurnsLeft = g_strInfiniteTurns;
		end
		Controls.MoveCityNumTurns:SetText(strTurnsLeft);
		Controls.MoveCityButton:RegisterCallback( Mouse.eLClick, OnMoveCityOptionSelected );

		Controls.MoveCityButton:RegisterCallback( Mouse.eRClick, function()
			Events.SearchForPediaEntry( g_moveCityPediaArticle.Description );
		end);
	else
		Controls.MoveCityGrid:SetHide(true);
	end

	if (g_IsProductionMode) then
		Controls.HeaderLabel:SetHide(false);
	else
		Controls.HeaderLabel:SetHide(true);
	end
	...
	if ( unitProduction ~= -1 )	then
		...
		-- Append trade unit warning if attempting to build trade unit in a city that has no more slots
		Controls.ProductionItemName:SetColor(COLOR_WHITE, 0);
		if (thisInfo.Trade == true and not bJustFinishedSomething) then
			if (city ~= nil and city:GetNumTradeRoutesAvailable() <= 0) then
				Controls.ProductionItemName:SetColor(COLOR_WARNING, 0);
				local warningStr = Locale.Lookup("TXT_KEY_BUILD_TRADE_UNIT_SLOTS_FULL_WARNING");
				strToolTip = strToolTip.."[NEWLINE][NEWLINE]"..warningStr;
			end
		end
	...
	if (g_IsProductionMode) then
		if (city:IsWater()) then
			totalOffset = totalOffset + MOVE_CITY_OFFSET_Y;
		end
	else
		totalOffset = totalOffset + PURCHASE_TABS_OFFSET_Y;
	end
...
function AddProductionButton( id, description, orderType, turnsLeft, column, isDisabled, ePurchaseYield )	
	...
	if column == 1 then -- we are a unit
		...
		strToolTip = Locale.ConvertTextKey(GetHelpTextForUnit(id, bIncludeRequirementsInfo, pCity));

		-- Append trade unit warning if attempting to build trade unit in a city that has no more slots
		if (thisUnitInfo.Trade == true) then
			if (pCity ~= nil and pCity:GetNumTradeRoutesAvailable() <= 0) then
				controlTable.UnitName:SetColor(COLOR_WARNING, 0);
				local warningStr = Locale.Lookup("TXT_KEY_BUILD_TRADE_UNIT_SLOTS_FULL_WARNING");
				strToolTip = strToolTip.."[NEWLINE][NEWLINE]"..warningStr;
			else
				controlTable.UnitName:SetColor(COLOR_WHITE, 0);
			end
		else
			controlTable.UnitName:SetColor(COLOR_WHITE, 0);
		end
		...
		if (isDisabled) then
			if (g_IsProductionMode) then
				...
				local strDisabledInfo : string = pCity:GetPurchaseUnitTooltip(id, ePurchaseYield);
				if (strDisabledInfo ~= nil and strDisabledInfo ~= "") then
					strToolTip = strToolTip .. "[NEWLINE][COLOR_WARNING_TEXT]" .. strDisabledInfo .. "[ENDCOLOR]";
				end
			end
	...
	elseif column == 2 or column == 3 then -- we are a building, wonder, or project
		...
		if orderType == OrderTypes.ORDER_MAINTAIN then
			...
			if orderType == OrderTypes.ORDER_CREATE then
				...
				-- Don't show the Move City project among other projects
				if (thisInfo.Type == g_moveCityProjecInfo.Type) then
					return;
				end
			...
			if (isDisabled) then
				...
				if (g_IsProductionMode) then
					...
					strDisabledInfo = pCity:GetPurchaseBuildingTooltip(id, ePurchaseYield);
				end
	...
	elseif column == 4 then -- processes
		...
		-- Tooltip
		local bIncludeRequirementsInfo = false;
		strToolTip = Locale.ConvertTextKey(GetHelpTextForProcess(id, bIncludeRequirementsInfo));
    ...
    if g_IsProductionMode then
		...
		elseif (ePurchaseYield == YieldTypes.YIELD_CAPITAL) then
			controlTable.NumTurns:SetText( turnsLeft.." [ICON_DIPLO_CAPITAL]");
		else
	...
	-- WRM: put switch check for capital here
		if (m_purchaseMode == PurchaseMode.ENERGY) then
			if (orderType == OrderTypes.ORDER_TRAIN) then
			...
			if (orderType == OrderTypes.ORDER_TRAIN) then
				ePurchaseEnum = g_PURCHASE_UNIT_CAPITAL;
			elseif (orderType == OrderTypes.ORDER_CONSTRUCT) then
				ePurchaseEnum = g_PURCHASE_BUILDING_CAPITAL;
			elseif (orderType == OrderTypes.ORDER_CREATE) then
				ePurchaseEnum = g_PURCHASE_PROJECT_CAPITAL;
			elseif (orderType == OrderTypes.ORDER_MAINTAIN) then
				print("SCRIPTING ERROR: Processes are not allowed to be purchased");
			end
		end
...
function InputHandler( uiMsg, wParam, lParam )
	if (UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_MOVE_CITY) then
		if (uiMsg == MouseEvents.LButtonUp or uiMsg == MouseEvents.PointerUp) then
			return OnMoveCityLButtonUp( wParam, lParam );
		elseif uiMsg == KeyEvents.KeyDown and wParam == Keys.VK_ESCAPE then
			RestoreSelectionMode();
			return true;
		end
...
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
function OnMouseOverHex(x : number, y : number)
	if (UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_MOVE_CITY) then
		RefreshMoveCityHoverHighlight(x, y);
	end
end
...
-- ===========================================================================
function DoMoveCityPlotSelection(eOrder, iData, projectInfo)

	UI.SetInterfaceMode(InterfaceModeTypes.INTERFACEMODE_MOVE_CITY);
	local city : object = GetCurrentCity();
	local cityX : number = city:GetX();
	local cityY : number = city:GetY();
	...
	-- Scan all adjacent plots
	for thisDirection : number = 0, (DirectionTypes.NUM_DIRECTION_TYPES - 1), 1 do
		local adjacentPlot : object = Map.PlotDirection(cityX, cityY, thisDirection);
		if (adjacentPlot) then
			local iHexID : table = ToHexFromGrid(Vector2(adjacentPlot:GetX(), adjacentPlot:GetY()));
			if (city:CanPlaceProjectAt(adjacentPlot:GetX(), adjacentPlot:GetY(), projectInfo.ID)) then
				Events.SerialEventHexHighlight(iHexID, true, Vector4(1.0, 1.0, 1.0, 1.0), g_MoveCityValidStyle);
			else
				Events.SerialEventHexHighlight(iHexID, true, Vector4(1.0, 1.0, 1.0, 1.0), g_MoveCityInvalidStyle);
			end
		end
	end
end

function OnMoveCityLButtonUp(wParam, lParam)
	local plot : object = Map.GetPlot(UI.GetMouseOverHex());
	local city : object = GetCurrentCity();
	if (plot and city and city:CanMoveCityToPlot(plot:GetPlotIndex())) then
		OnPlotSelectedForProject(plot:GetPlotIndex());
	end
	return true;
end

function RefreshMoveCityHoverHighlight(x : number, y : number)
	Events.ClearHexHighlightStyle(g_MoveCityHoverStyle);
	local city : object = GetCurrentCity();
	if (city and city:CanPlaceProjectAt(x, y, g_moveCityProjecInfo.ID)) then
		local iHexID : table = ToHexFromGrid(Vector2(x, y));
		Events.SerialEventHexHighlight(iHexID, true, Vector4(1.0, 1.0, 1.0, 1.0), g_MoveCityHoverStyle);
	end
end

function ClearMoveCityHighlights()
	Events.ClearHexHighlightStyle(g_MoveCityValidStyle);
	Events.ClearHexHighlightStyle(g_MoveCityInvalidStyle);
	Events.ClearHexHighlightStyle(g_MoveCityHoverStyle);
end
...
function DoProjectPlotSelection( eOrder, iData, projectInfo )	
	...
	local aProjectPlots = { pCity:GetProjectPlotsList(projectInfo.ID) };

	-- Do selection buttons
	for i = 1, #aProjectPlots, 1 do
		local plot = aProjectPlots[i];
		if (plot ~= nil) then
			local hexPos = ToHexFromGrid( Vector2( plot:GetX(), plot:GetY() ) );
			local worldPos = HexToWorld( hexPos );
			if (pCity:CanPlaceProjectAt(plot:GetX(), plot:GetY(), projectInfo.ID)) then
				...
				local tipKey : string = projectInfo.PlotPlacementToolTip;
				if (tipKey == "") then
					tipKey = "TXT_KEY_PLACE_WONDER_HERE";
				end
				controlTable.PlotSelectAnchoredButton:SetToolTipString( Locale.ConvertTextKey(tipKey) );
...
function GetUIIconsForBuilding( iBuildingID, pCity, isShowingAmt )
	...
	for yieldInfo in GameInfo.Yields() do
		...
		iFlatYield = iFlatYield + GetPlayerPerkBuildingFlatYieldChanges(g_activePerkTypes, Game.GetActivePlayer(), iBuildingID, eYield);
		if (pCity ~= nil) then
		...
		iModYield = iModYield + GetPlayerPerkBuildingPercentYieldChanges(g_activePerkTypes, Game.GetActivePlayer(), iBuildingID, eYield);
	...
	iHealthTotal = iHealthTotal + GetPlayerPerkBuildingFlatHealthChanges(g_activePerkTypes, Game.GetActivePlayer(), iBuildingID);
	...
	iHealthMod = iHealthMod + GetPlayerPerkBuildingPercentHealthChanges(g_activePerkTypes, Game.GetActivePlayer(), iBuildingID);
...
function Initialize()
	...
	m_purchaseTabs = CreateTabs(Controls.PurchaseSelectorTabs, 370, 32);
	m_purchaseTabs.AddTab(Controls.PurchaseWithEnergyTab, function()
		if (m_purchaseMode ~= PurchaseMode.ENERGY) then
			m_purchaseMode = PurchaseMode.ENERGY;
			UpdateWindow();
		end
	end);
	m_purchaseTabs.AddTab(Controls.PurchaseWithCapitalTab, function()
		if (m_purchaseMode ~= PurchaseMode.CAPITAL) then
			m_purchaseMode = PurchaseMode.CAPITAL;
			UpdateWindow();
		end
	end);
	m_purchaseTabs.CenterAlignTabs();
	m_purchaseTabs.SelectTab(Controls.PurchaseWithEnergyTab);
