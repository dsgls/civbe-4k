...
function UpdateWindow()
	...
	for i,v in ipairs(g_sponsors) do
		...
		local descriptionStr : string = "";

		-- Help text for character trait
		local traitText = nil;
		if (g_traitsQuery ~= nil) then
			for row in g_traitsQuery(sponsor.Personality) do
				traitText = row.Help;
			end
		end
		if (traitText ~= nil) then
			descriptionStr = descriptionStr .. Locale.Lookup(traitText);
		end

		for row in GameInfo.Civilization_Start_In_Water() do
			if (row.CivilizationType == sponsor.Type) then
				descriptionStr = descriptionStr.."[NEWLINE]"..Locale.Lookup("TXT_KEY_SPONSOR_MENU_WATER_START");
			end
		end

		instance.DescriptionLabel:SetText(descriptionStr);
	...
	for _,uiItem in pairs(uiList) do
		...
		uiItem.DescriptionLabel:SetWrapWidth( sizeX - 150 );
	end
...
function RefreshCivs()
	...
	g_traitsQuery = DB.CreateQuery([[SELECT Description, Help FROM PersonalityTraits inner join 
										 Personalities ON PersonalityTraits.Type = Personalities.UniquePersonalityTrait 
										 WHERE Personalities.Type = ? LIMIT 1]]);
