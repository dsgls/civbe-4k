...
local CachedUnitAffinityPrereqs	: table = {};
local CachedBuildingAffinityPrereqs	: table = {};
local CachedProjectAffinityPrereqs	: table = {};
...
CivilopediaCategory[CategoryUnits].PopulateList = function()
	...
	for unit in GameInfo.Units() do
		...
		local ignore : boolean = unitsToIgnore[unit.Type];
...
CivilopediaCategory[CategoryAffinities].PopulateList = function()
	...
	for affinity in GameInfo.Affinity_Types() do
		local article : table = {};
		local name : string = Locale.ConvertTextKey("TXT_KEY_PEDIA_AFFINITY_PERKS_LABEL", affinity.Description);
		local entryID : number = affinity.ID + 1000;
		local articleType : string = affinity.Type .. "_PERKS";
...
CivilopediaCategory[CategoryDiplomacy].PopulateList = function()
	sortedList[CategoryDiplomacy] = {};
	
	-- first Personality Traits
	sortedList[CategoryDiplomacy][1] = {};
	local tableid = 1;

	for row in GameInfo.PersonalityTraits() do	
		local article = {};
		local name = Locale.ConvertTextKey( row.Description )
		article.entryName = name;
		article.entryID = row.ID;
		...
		sortedList[CategoryDiplomacy][1][tableid] = article;
		tableid = tableid + 1;
		
		-- index by various keys
		searchableList[Locale.ToLower(name)] = article;
		searchableTextKeyList[row.Description] = article;
		m_categorizedListOfArticles[(CategoryDiplomacy * MAX_ENTRIES_PER_CATEGORY) + row.ID] = article;
	end
	
	-- sort this list alphabetically by localized name
	table.sort(sortedList[CategoryDiplomacy][1], Alphabetically);
			
	-- next Foreign Policies
	sortedList[CategoryDiplomacy][2] = {};
	tableid = 1;

	for row in GameInfo.ForeignPolicies() do	
		local article = {};
		local name = Locale.ConvertTextKey( row.Description )
		article.entryName = name;
		article.entryID = row.ID + 1000;
		...
		sortedList[CategoryDiplomacy][2][tableid] = article;
		tableid = tableid + 1;
		
		-- index by various keys
		searchableList[Locale.ToLower(name)] = article;
		searchableTextKeyList[row.Description] = article;
		m_categorizedListOfArticles[(CategoryDiplomacy * MAX_ENTRIES_PER_CATEGORY) + row.ID + 1000] = article;
	end

	-- sort this list alphabetically by localized name
	table.sort(sortedList[CategoryDiplomacy][2], Alphabetically);
end
...
CivilopediaCategory[CategoryArtifacts].PopulateList = function()
	...
	-- first Artifacts
	sortedList[CategoryArtifacts][1] = {};
	local tableid = 1;
	...
	for row in GameInfo.Artifacts() do	
		...
		sortedList[CategoryArtifacts][1][tableid] = article;
		tableid = tableid + 1;
	...
	-- sort this list alphabetically by localized name
	table.sort(sortedList[CategoryArtifacts][1], Alphabetically);
			
	-- next Artifact Reward Perks
	sortedList[CategoryArtifacts][2] = {};
	tableid = 1;
	...
	for row in GameInfo.ArtifactRewards() do
		...
		sortedList[CategoryArtifacts][2][tableid] = article;
		tableid = tableid + 1;
...
CivilopediaCategory[CategoryDiplomacy].DisplayHomePage = function()
	ClearArticle();
	Controls.ArticleID:SetText( Locale.ConvertTextKey( "TXT_KEY_PEDIA_DIPLOMACY_PAGE_LABEL" ));	
...
CivilopediaCategory[CategoryArtifacts].DisplayHomePage = function()
	ClearArticle();
	Controls.ArticleID:SetText( Locale.ConvertTextKey( "TXT_KEY_PEDIA_ARTIFACTS_PAGE_LABEL" ));	
...
CivilopediaCategory[CategoryTech].SelectArticle = function( techID, shouldAddToList )
	...
	if techID ~= -1 then
		...
		for row in GameInfo.Technology_Affinities( condition ) do
			local affinityInfo : table = GameInfo.Affinity_Types[row.AffinityType];
...
CivilopediaCategory[CategoryUnits].SelectArticle = function( unitID, shouldAddToList )
	...
	if unitID ~= -1 then
		...
		for row in GameInfo.Unit_FreePromotions( condition ) do
			...
			if promotion then
				...
				if thisPromotionInstance then
					...
					local promotionText : string = "[COLOR_YELLOW]" .. Locale.ConvertTextKey( promotion.Description ) .. "[ENDCOLOR][NEWLINE][NEWLINE]" .. Locale.ConvertTextKey( promotion.Help );
		...
		if #abilityLines > 0 then
			local abilitiesString : string = table.concat(abilityLines, "[NEWLINE]");
		...
		if (unitAffinityPrereq ~= nil) then
			local gameInfoText : string = "";
			local firstEntry : boolean = true;

			for affinityType, level in pairs(unitAffinityPrereq) do
				local affinityInfo : table = GameInfo.Affinity_Types[affinityType];
				local prereqString : string = Locale.ConvertTextKey("TXT_KEY_AFFINITY_LEVEL_REQUIRED", affinityInfo.ColorType, level, affinityInfo.IconString, affinityInfo.Description);
			...
			local affinityHeader : string = Locale.ConvertTextKey("TXT_KEY_PEDIA_CATEGORY_15_LABEL") .. ":";
	...
	if rawCivID < 1000 then
		...
		if civID ~= -1 then
			...
			--if thisCiv and thisCiv.Type ~= "CIVILIZATION_MINOR" and thisCiv.Type ~= "CIVILIZATION_ALIEN" and row.CivilizationType ~= "CIVILIZATION_NEUTRAL_PROXY" then
				...
				-- update the game info
				local personalityInfo : table = GameInfo.Personalities[thisCiv.Personality];
				if (personalityInfo ~= nil) then
					local uniqueTrait : table = GameInfo.PersonalityTraits[personalityInfo.UniquePersonalityTrait];
	...
	elseif rawTerrainID < 4000 then
		...
		if planetID ~= -1 then
			...
			if planetInfo then
				...
				-- Name
				local name = Locale.ConvertTextKey( planetInfo.Description )
				Controls.ArticleID:SetText( name );
...
CivilopediaCategory[CategoryImprovements].SelectArticle = function( improvementID, shouldAddToList )
	...
	if improvementID ~= -1 and improvementID < 1000 then
		...
		if thisImprovement then
			...
			if energyMaintenance > 0 then
				Controls.MaintenanceLabel:SetText( Locale.ConvertTextKey("SIMPLE_NUM_NAMED_YIELD", energyMaintenance, "[ICON_ENERGY]", "TXT_KEY_YIELD_ENERGY") );
				Controls.MaintenanceFrame:SetHide( false );
			end
			...
			if iHealth > 0 then
				Controls.HealthLabel:SetText( Locale.ConvertTextKey("SIMPLE_NUM_NAMED_YIELD", iHealth, "[ICON_HEALTH_1]", "TXT_KEY_HEALTH") );
				Controls.HealthFrame:SetHide( false );
			elseif iUnhealth > 0 then
				Controls.HealthLabel:SetText( Locale.ConvertTextKey("SIMPLE_NUM_NAMED_YIELD", iUnhealth, "[ICON_HEALTH_4]", "TXT_KEY_UNHEALTH") );
				Controls.HealthFrame:SetHide( false );
			end
	...
	if conceptID ~= -1 then
		...
		if (conceptID >= 1000) then
			...
			if (affinityInfo ~= nil) then
				-- Name
				Controls.ArticleID:SetText( Locale.ConvertTextKey("TXT_KEY_PEDIA_AFFINITY_PERKS_LABEL", affinityInfo.Description) );

				-- Summary
				local summaryLines : table = {};

				local mainColumn : string = "";
				local hybridOptions : table = {};
				...
				for row in GameInfo.Affinity_Perks(mainColumn .. " > 0") do
					local perkInfo : table = GameInfo.PlayerPerks[row.PlayerPerk];
					local perkStr : string = Locale.ConvertTextKey("TXT_KEY_PEDIA_AFFINITY_PERKS_AT_LEVEL", row[mainColumn], affinityInfo.IconString, affinityInfo.Description);

					-- Hybrid?
					local hybridAffinity : table = nil;
	...
	if entryID < 1000 then
		...
		if traitID ~= -1 then
			...
			if traitInfo then
				...
				-- Name
				local name = Locale.ConvertTextKey( traitInfo.Description )
				Controls.ArticleID:SetText( name );

				local traitCategory : table = GameInfo.PersonalityTraitCategories[traitInfo.TraitCategoryType];
		...
		if policyID ~= -1 then
			...
			if policyInfo then
				...
				-- Name
				local name = Locale.ConvertTextKey( policyInfo.Description )
				Controls.ArticleID:SetText( name );		
				...
				for row in GameInfo.ForeignPolicies_Perks{ ForeignPolicyType = policyInfo.Type } do
					...
					if (rowPerk ~= nil) then
						...
						local relationshipInfo : table = GameInfo.RelationshipLevels[row.RelationshipLevelType];
				...
				-- tech visibility
				g_RevealTechsManager:DestroyInstances();
				local unlockTraitType : string = nil;
				...
				local unlockTrait : table = GameInfo.PersonalityTraits[unlockTraitType];
	...
	if entryID < 1000 then
		...
		if artifactID ~= -1 then
			...
			if (artifactInfo ~= nil) then
				-- Name
				local name = Locale.ConvertTextKey( artifactInfo.Description );
				Controls.ArticleID:SetText( name );
	...
	-- Artifact Reward Perks
		...
		if rewardID ~= -1 then
			...
			if (rewardInfo ~= nil) then
				-- Name
				local name = Locale.ConvertTextKey( rewardInfo.Description );
				Controls.ArticleID:SetText( name );

				-- Portrait
				if rewardInfo.BuildingReward ~= nil then
					local building : table = GameInfo.Buildings[rewardInfo.BuildingReward];
...
CivilopediaCategory[CategoryDiplomacy].DisplayList = function()
