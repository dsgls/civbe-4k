...
function UpdateData()
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0 or pPlayer:GetNumOutposts() > 0) then
			...
			local affinityCallback : ifunction = function()
				Events.SerialEventGameMessagePopup
				{
					Type = ButtonPopupTypes.BUTTONPOPUP_AFFINITY_OVERVIEW,
					data1 = -1,
					data2 = -1,
				};
			end

			Controls.Harmony:RegisterCallback(Mouse.eLClick, affinityCallback);
			Controls.Purity:RegisterCallback(Mouse.eLClick, affinityCallback);
			Controls.Supremacy:RegisterCallback(Mouse.eLClick, affinityCallback);
			...
			-----------------------------
			-- Update Diplo Capital
			-----------------------------
			local capitalStr : string;
			local capital : number = pPlayer:GetDiplomaticCapital();
			local netCapital : number = pPlayer:GetNetDiplomaticCapitalPerTurn();
			if (netCapital > 0) then
				capitalStr = string.format("%i (+%i)", capital, netCapital);
			elseif (netCapital == 0) then
				capitalStr = string.format("%i (0)", capital);
			else
				capitalStr = string.format("%i [COLOR_RED](-%i)", capital, netCapital);
			end
			diploCapitalStr = "[ICON_DIPLO_CAPITAL]" .. capitalStr;
			Controls.DiplomaticCapitalString:SetText(diploCapitalStr);

			SetAutoWidthGridButton(Controls.DiplomaticCapitalString, diploCapitalStr, BUTTON_PADDING);
			...
			for pResource in GameInfo.Resources() do
				...
				if (Game.GetResourceClassType(iResourceLoop) == GameInfo.ResourceClasses["RESOURCECLASS_STRATEGIC"].ID) then
					...
					bShowResource	= pTeam:IsResourceRevealed(iResourceLoop) or pTeam:IsResourceUIVisible(iResourceLoop);
...
function OnDiploCapitalClicked()
	Events.SerialEventGameMessagePopup{Type = ButtonPopupTypes.BUTTONPOPUP_DIPLOMACY_OVERVIEW, Data2 = Game.GetActivePlayer()};
end
Controls.DiplomaticCapitalString:RegisterCallback( Mouse.eLClick, OnDiploCapitalClicked );
...
function DoInitTooltips()
	...
	Controls.DiplomaticCapitalString:RegisterMouseOverCallback(DiplomaticCapitalTipHandler);
...
function ScienceTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
		...
		local pTeam = Teams[pPlayer:GetTeam()];
		local pCity = UI.GetHeadSelectedCity();
		local pScienceInfo = GameInfo.Yields[YieldTypes.YIELD_SCIENCE];
	
		local iSciencePerTurn = pPlayer:GetScience() * 100;
		...
		-- Science
		strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE", iSciencePerTurn / 100);
		...
		local iScienceFromCulture = pPlayer:GetScienceFromCultureTimes100();
		if (iScienceFromCulture ~= 0) then
			...
			strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE_FROM_CULTURE", iScienceFromCulture / 100);
		end
	
		-- Science from Diplomacy Rate
		local iScienceFromDiplomacy = pPlayer:GetScienceFromDiplomacyTimes100();
		if (iScienceFromDiplomacy > 0) then			
			...
			strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_SCIENCE_FROM_DIPLOMACY", iScienceFromDiplomacy / 100);
		end

		-- Science from Affinity
		local iScienceFromAffinity = 0;
		for row in GameInfo.Affinity_Types() do
			iScienceFromAffinity = iScienceFromAffinity + (pPlayer:GetYieldPerTurnFromAffinityLevel(row.ID, YieldTypes.YIELD_SCIENCE) * 100);
		end
		if (iScienceFromAffinity > 0) then			
			-- Add separator for non-initial entries
			if (bFirstEntry) then
				bFirstEntry = false;
			else
			...
			strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_YIELD_FROM_AFFINITY_LEVEL", iScienceFromAffinity / 100, pScienceInfo.IconString, pScienceInfo.Description);
		end
		...
		if (iUncategorizedScience > 0) then
			...
			strText = strText .. "[ICON_BULLET]"..Locale.ConvertTextKey("TXT_KEY_TP_YIELD_FROM_UNCATEGORIZED", iUncategorizedScience / 100);
		end
...
function GoldTipHandler( control )
	...
	local iUncategorizedIncome = fTotalIncome - fExplicitIncome;	-- Bucket for any energy income that isn't explicitly called out on its own
...
function HealthTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HEALTH)) then
		...
		local iNationalSecProject	= pPlayer:GetHealthFromNationalSecurityProject();
		local iDiplomacy			= pPlayer:GetHealthFromDiplomacy();

		local iUnaccountedFor		= iTotalHealth - (iHandicapHealth + iPoliciesHealth + iCityHealth + iTradeRouteHealth + iExtraCityHealth + iNationalSecProject + iDiplomacy);
		...
		if (iNationalSecProject ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText .. "  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_NATL_SECURITY_PROJECTS", iNationalSecProject);
		end
		if (iDiplomacy ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText .. "  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_DIPLOMACY", iDiplomacy);
		end
...
-- ===========================================================================
-- Diplomatic Capital ToolTip
-- ===========================================================================
function DiplomaticCapitalTipHandler(control)
	local player : object = Players[Game.GetActivePlayer()];

	-- Basic explanation of Diplo Capital
	local explanation : string = Locale.ConvertTextKey("TXT_KEY_TP_DIPLO_CAPITAL_EXPLANATION");	

	-- Stockpile
	local stockpiledCapital : number = player:GetDiplomaticCapital();
	local stockpileStr : string = Locale.ConvertTextKey("TXT_KEY_TP_AVAILABLE_CAPITAL", stockpiledCapital);

	-- Income
	local income : number = player:GetTotalDiplomaticCapitalPerTurn();
	local incomeStr : string = "[COLOR_GREEN]" .. Locale.ConvertTextKey("TXT_KEY_TP_TOTAL_INCOME", income) ..  "[ENDCOLOR]";
	-- Income source breakdown
	local incomeFromCities : number = player:GetDiplomaticCapitalPerTurnFromCities();
	incomeStr = incomeStr .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_CITY_OUTPUT", incomeFromCities);
	local incomeFromAgreements : number = player:GetDiplomaticCapitalPerTurnFromAgreements();
	incomeStr = incomeStr .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_AGREEMENTS_OUTPUT", incomeFromAgreements);
	local incomeFromBlackMarket : number = player:GetTotalDiploCapitalIncomeFromBlackMarket();
	incomeStr = incomeStr .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_BLACK_MARKET_INCOME", incomeFromBlackMarket);

	-- Spending
	local totalSpending : number = player:GetTotalDiplomaticCapitalCostsPerTurn();
	local spendingStr : string = "[COLOR_RED]" .. Locale.ConvertTextKey("TXT_KEY_TP_TOTAL_EXPENSES", totalSpending) ..  "[ENDCOLOR]";

	local spendingOnAgreements : number = player:GetTotalDiplomaticCapitalCostsPerTurnFromAgreements();	
	spendingStr = spendingStr .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_AGREEMENTS_SPENDING", spendingOnAgreements);

	local spendingOnBlackMarket : number = player:GetTotalDiploCapitalSpentInBlackMarket();
	spendingStr = spendingStr .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_BLACK_MARKET_SPENDING", spendingOnBlackMarket);

	-- Assemble tooltip
	local toolTip : string = explanation;
	toolTip = toolTip .. "[NEWLINE][NEWLINE]";
	toolTip = toolTip .. incomeStr;
	toolTip = toolTip .. "[NEWLINE][NEWLINE]";
	toolTip = toolTip .. spendingStr;
