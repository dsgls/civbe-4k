...
function UpdateUnitStats(pUnit)
	...
	local szMoveStr = "[ICON_MOVES] " .. math.max(max_moves, 0); 
...
function UpdateCombatOddsUnitVsSite(pMyUnit, pPlot)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			-- My Strength
			if (bRanged) then
				Controls.MyStrength:SetText(Locale.Lookup("TXT_KEY_EUPANEL_UNIT_YOUR_RANGE_STRENGTH"));
			else
				Controls.MyStrength:SetText(Locale.Lookup("TXT_KEY_EUPANEL_UNIT_YOUR_STRENGTH"));
			end
			...
			-- Attack bonus from starting turn invisible
			if (pMyUnit:HasStartedTurnInvisible(pTheirPlayer:GetTeam())) then
				iModifier = pMyUnit:GetStartedTurnInvisibleAttackModifier();
				if (iModifier ~= 0) then
					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ATTACK_FROM_INVISIBILITY_MOD" );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );	
				end
			end
			...
			if (not pPlot:IsFriendlyTerritory(iMyPlayer)) then
				...
				-- Neutral territory
				if (pPlot:IsNeutralTerritory(iMyPlayer)) then	
					iModifier = pMyPlayer:GetUnitStrengthNeutralTerritoryMod();
					if (iModifier ~= 0) then
						controlTable = g_MyCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_NEUTRAL_TERRITORY" );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
					end
				-- Enemy territory
				else
					iModifier = pMyPlayer:GetUnitStrengthEnemyTerritoryMod();
					if (iModifier ~= 0) then
						controlTable = g_MyCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ENEMY_TERRITORY" );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
					end
				end
			end
			...
			-- Within orbital coverage modifier
			iModifier = pMyUnit:GetCombatModFromOrbitalCoverage();
			if (iModifier ~= 0) then
				controlTable = g_MyCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ORBITAL_COVERAGE" );
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	if (pMyUnit ~= nil) then
		...
		local iMyStrength = 0;
		local iTheirStrength = 0;
		local bRanged : boolean = false;
		local bAir : boolean = false;
		local bOrbital : boolean = false;
		local iNumVisibleAAUnits = 0;
		local bInterceptPossible = false;
		...
		-- Air/Orbital?
		bAir = pMyUnit:GetDomainType() == DomainTypes.DOMAIN_AIR;
		bOrbital = pMyUnit:IsOrbitalUnit() and pMyUnit:IsInOrbit();
		...
		if (iMyStrength > 0) then
			...
			if (pToPlot:IsFriendlyTerritory(c)) then
				...
				-- Player-level territory mod
				iModifier = pMyPlayer:GetUnitStrengthFriendlyTerritoryMod();
				if (iModifier ~= 0) then
					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_FRIENDLY_TERRITORY" );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
				end
			end
			...
			if (not pToPlot:IsFriendlyTerritory(iMyPlayer)) then
				...
				-- Neutral territory
				if (pToPlot:IsNeutralTerritory(iMyPlayer)) then	
					iModifier = pMyPlayer:GetUnitStrengthNeutralTerritoryMod();
					if (iModifier ~= 0) then
						controlTable = g_MyCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_NEUTRAL_TERRITORY" );
			...
			-- Temporary Turn Buffs
			iModifier = pMyUnit:GetTemporaryTurnBuff(BuffChannels.BUFFCHANNEL_ATTACK_STRENGTH_MOD);
			if (iModifier ~= 0) then
				controlTable = g_MyCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_BUFFCHANNEL_ATTACK_MOD");
			...
			if (iModifier ~= 0) then
				controlTable = g_MyCombatDataIM:GetInstance();
				local unitClassType = GameInfo.UnitClasses[pTheirUnit:GetUnitClassType()];
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_BONUS_VS_CLASS", unitClassType.Description );
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
			end
			...
			if (pTheirUnit:GetUnitCombatType() ~= -1) then
				...
				if (iModifier ~= 0) then
					controlTable = g_MyCombatDataIM:GetInstance();
					local unitClassType = GameInfo.UnitCombatInfos[pTheirUnit:GetUnitCombatType()];
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_BONUS_VS_CLASS", unitClassType.Description );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
				end
			end
			...
			-- attackUnfortifiedMod
			else
				iModifier = pMyUnit:AttackUnfortifiedModifier();

				if (iModifier ~= 0) then
					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_BONUS_VS_UNFORT_UNITS" );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
				end
			end
	...
	--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_FORTIFY_MOD", iModifier));
				...
				-- Within orbital coverage modifier
				iModifier = pTheirUnit:GetCombatModFromOrbitalCoverage(false);
				if (iModifier ~= 0) then
					controlTable = g_TheirCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ORBITAL_COVERAGE" );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
	...
	--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_EXTRA_STRENGTH", iModifier));
				...
				if (pToPlot:IsFriendlyTerritory(iTheirPlayer)) then
					...
					-- Player-level territory mod
					iModifier = pTheirPlayer:GetUnitStrengthFriendlyTerritoryMod();
					if (iModifier ~= 0) then
						controlTable = g_TheirCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_FRIENDLY_TERRITORY" );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
				...
				if (not pToPlot:IsFriendlyTerritory(iTheirPlayer)) then
					...
					-- Neutral territory
					if (pToPlot:IsNeutralTerritory(iTheirPlayer)) then	
						iModifier = pTheirPlayer:GetUnitStrengthNeutralTerritoryMod();
						if (iModifier ~= 0) then
							controlTable = g_TheirCombatDataIM:GetInstance();
							controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_NEUTRAL_TERRITORY" );
							controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
					...
					-- Enemy territory
						...
						iModifier = pTheirPlayer:GetUnitStrengthEnemyTerritoryMod();
						if (iModifier ~= 0) then
							controlTable = g_TheirCombatDataIM:GetInstance();
							controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ENEMY_TERRITORY" );
							controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
				...
				-- Temporary Turn Buffs
				iModifier = pTheirUnit:GetTemporaryTurnBuff(BuffChannels.BUFFCHANNEL_DEFENSE_STRENGTH_MOD);
				if (iModifier ~= 0) then
					controlTable = g_TheirCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_BUFFCHANNEL_DEFENSE_MOD");
					controlTable.Value:SetText(GetFormattedText(strText, iModifier, false, true));
				end

				-- Defense vs Attack Type --

				-- vs Orbital
				if (bOrbital) then
					iModifier = pTheirUnit:GetOrbitalDefenseModifier();
					if (iModifier ~= 0) then
						controlTable = g_TheirCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ORBITAL_DEFENSE_BONUS" );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
					end
				-- vs Air
				elseif (bAir) then
					iModifier = pTheirUnit:GetAirDefenseModifier();
					if (iModifier ~= 0) then
						controlTable = g_TheirCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_AIR_DEFENSE_BONUS" );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
					end
				-- vs Ranged
				elseif (bRanged) then
				...
				-- vs Melee
					...
					iModifier = pTheirUnit:GetMeleeDefenseModifier();
					if (iModifier ~= 0) then
						controlTable = g_TheirCombatDataIM:GetInstance();
						controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_MELEE_DEFENSE_BONUS" );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
					end
				end
				...
				if (iModifier ~= 0) then
					controlTable = g_TheirCombatDataIM:GetInstance();
					local unitClassType = GameInfo.UnitClasses[pMyUnit:GetUnitClassType()];
					controlTable.Text:LocalizeAndSetText(  "TXT_KEY_EUPANEL_BONUS_VS_CLASS", unitClassType.Description );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
				end
				...
				if (pMyUnit:GetUnitCombatType() ~= -1) then
					...
					if (iModifier ~= 0) then
						controlTable = g_TheirCombatDataIM:GetInstance();
						local unitClassType = GameInfo.UnitCombatInfos[pMyUnit:GetUnitCombatType()];
						controlTable.Text:LocalizeAndSetText(  "TXT_KEY_EUPANEL_BONUS_VS_CLASS", unitClassType.Description );
						controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
					end
				end
...
--		Site vs Unit
-- ===========================================================================
function UpdateCombatOddsSiteVsUnit(mySite, theirUnit)
	...
	--Set Initial Values
	local myCityMaxHP = mySite:GetMaxHitPoints();
	local myCityCurHP = mySite:GetDamage();
	local myCityDamageInflicted = mySite:GetRangedDamageVsUnit(theirUnit, nil);
	local myCityStrength = mySite:GetCombatStrengthWhenAttackingUnit(theirUnit);
	...
	local theirUnitStrength = theirUnit:GetRangeCombatDefense();
	...
	if (theirUnit:IsCombatUnit()) then
		...
		local myPlayerID	= mySite:GetOwner();
...
--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_FORTIFY_MOD", iModifier));
		...
		-- Within orbital coverage modifier
		iModifier = theirUnit:GetCombatModFromOrbitalCoverage(false);
		if (iModifier ~= 0) then
			controlTable = g_TheirCombatDataIM:GetInstance();
			controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_ORBITAL_COVERAGE" );
			controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
...
--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_EXTRA_STRENGTH", iModifier));
		...
		if (not theirPlot:IsFriendlyTerritory(iTheirPlayer)) then
			...
			-- Neutral territory
			if (theirPlot:IsNeutralTerritory(iTheirPlayer)) then	
				iModifier = pTheirPlayer:GetUnitStrengthNeutralTerritoryMod();
				if (iModifier ~= 0) then
					controlTable = g_TheirCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_NEUTRAL_TERRITORY" );
		...
		-- Defense Modifier against City Strike
		if (bRanged) then
			iModifier = theirUnit:GetCityStrikeDefenseModifier();
			if (iModifier ~= 0) then
				controlTable = g_TheirCombatDataIM:GetInstance();
				controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_CITY_STRIKE_DEFENSE_BONUS" );
				controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
			end
		end

		-- Temporary Turn Buffs
		iModifier = theirUnit:GetTemporaryTurnBuff(BuffChannels.BUFFCHANNEL_DEFENSE_STRENGTH_MOD);
		if (iModifier ~= 0) then
			controlTable = g_TheirCombatDataIM:GetInstance();
			controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_BUFFCHANNEL_DEFENSE_MOD");
			controlTable.Value:SetText(GetFormattedText(strText, iModifier, false, true));
		...
		local siteCity : object = mySite:GetPlot():GetPlotCity();
		if (siteCity ~= nil) then
			...
			if (theirUnit:IsAlien()) then
				...
				iModifier = iModifier + siteCity:GetStrengthModVsAliens();
		...
		-- Defending Unit
		if (mySite:GetDefendingUnit() ~= nil) then		
...
function OnMouseOverHex( hexX, hexY )
	...
	if (pPlot ~= nil) then
		...
		-- Fighting with a Melee Unit
		if (pHeadUnit ~= nil) then
			if (iInterfaceMode == InterfaceModeTypes.INTERFACEMODE_AIR_SWEEP or iInterfaceMode == InterfaceModeTypes.INTERFACEMODE_SUPPORT_ACTION) then
				-- Don't show panel for air sweeps, air sweep combat works differently
				-- Also don't show panel for support actions.
			elseif (pHeadUnit:IsCombatUnit() and (pHeadUnit:IsRanged() and pHeadUnit:IsEmbarked()) == false) and ((pHeadUnit:IsRanged() and pHeadUnit:IsRangeAttackOnlyInDomain() and not pPlot:IsWater()) == false) then
				...
				if (pPlot:IsRevealed(iTeam, false)) then
					...
					if (pPlot:IsStrategicSite()) then
						...
						-- This check is for land units which can't attack a water site.
						local canAttackSite = true;
						if (pHeadUnit:GetDomainType() == DomainTypes.DOMAIN_LAND 
								and pPlot:IsWater() 
								and not pHeadUnit:IsRanged()
								and not pHeadUnit:CanMoveAllTerrain() 
								and not pHeadUnit:IsHoveringUnit()
								and not pPlot:IsAllowsWalkWater()) then
							canAttackSite = false;
						elseif (pHeadUnit:GetDomainType() == DomainTypes.DOMAIN_SEA
								and not pPlot:IsWater() 
								and not pHeadUnit:IsRanged()
								and not pHeadUnit:CanMoveAllTerrain() 
								and not pHeadUnit:IsHoveringUnit()) then
							canAttackSite = false;
						end

						if (canAttackSite and (pPlot:GetPlotStrategicSite():CanTeamAttackSite(iTeam) or (UIManager:GetAlt() and pPlot:GetOwner() ~= iTeam))) then
							...
							for i = 0, iNumUnits do
								...
								if (pUnit ~= nil and not pUnit:IsInvisible(iTeam, false)) then
									...
									--if (pUnit:GetDomainType() ~= DomainTypes.DOMAIN_AIR) then
										...
										if ( (pHeadUnit:GetDomainType() == pUnit:GetDomainType() and (not pHeadUnit:IsEmbarked() and not pUnit:IsEmbarked()) )
											or pHeadUnit:IsRanged()
											or (pHeadUnit:GetDomainType() == DomainTypes.DOMAIN_SEA and pUnit:IsEmbarked())
											or pHeadUnit:CanMoveAllTerrain()
											or ((pHeadUnit:IsAmphibiousUnit() or pHeadUnit:IsHoveringUnit()) and (pUnit:GetDomainType() == DomainTypes.DOMAIN_LAND or pUnit:GetDomainType() == DomainTypes.DOMAIN_SEA) ) ) then
		...
		-- Fighting with a City
		elseif((pHeadCity ~= nil and pHeadCity:CanRangeStrikeNow()) or
			   (selectedSite ~= nil and selectedSite:CanRangeStrikeNow()))
		then
					...
					if (theirUnit ~= nil and not theirUnit:IsInvisible(myTeamID, false)) then
						...
						if (theirUnit:GetDomainType() ~= DomainTypes.DOMAIN_AIR) then
							...
							if (myTeam:IsAtWar(theirUnit:GetTeam()) or (UIManager:GetAlt() and theirUnit:GetOwner() ~= myTeamID)) then
								...
								if pHeadCity ~= nil then
									selectedSite = pHeadCity:GetStrategicSite();
								end

								UpdateCombatOddsSiteVsUnit(selectedSite, theirUnit);
