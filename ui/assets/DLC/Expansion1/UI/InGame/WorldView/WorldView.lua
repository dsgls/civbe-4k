...
local g_invisibleRevealWarning = "InvisibilityVisiblePlot";
...
InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_SUPPORT_ACTION][KeyEvents.KeyDown] = 
function( wParam, lParam )
	if ( wParam == Keys.VK_ESCAPE ) then
		UI.SetInterfaceMode(InterfaceModeTypes.INTERFACEMODE_SELECTION);
		return true;
...
function AirStrike( wParam, lParam )
	...
	-- Don't "need war" in neutral territory (Fluid Hostility).		
	local needsWar = plot:GetOwner() ~= -1;
	needsWar = true;

	-- should handle the case of units bombarding tiles when they are already at war
	if pHeadSelectedUnit and pHeadSelectedUnit:CanRangeStrikeAt(plotX, plotY, needsWar, true) then
		-- Set temporary enemies in fluid hostility case.
		local owner = pHeadSelectedUnit:GetOwner();
		local bestDefender = plot:GetBestDefender(-1, owner, pHeadSelectedUnit);
		if (bestDefender ~= nil and pHeadSelectedUnit:CanBeTemporaryEnemy(bestDefender:GetOwner())) then
			bestDefender:SetIsTemporaryEnemy(owner, true);
		end
...
function RangeAttack( wParam, lParam )
	...
	if pHeadSelectedUnit and pHeadSelectedUnit:CanRangeStrikeAt(plotX, plotY, needsWar, true) then
		...
		-- Set temporary enemies for Fluid Hostility
		local owner = pHeadSelectedUnit:GetOwner();
		local bestDefender = plot:GetBestDefender(-1, owner, pHeadSelectedUnit);
		if (bestDefender ~= nil and pHeadSelectedUnit:CanBeTemporaryEnemy(bestDefender:GetOwner())) then
			bestDefender:SetIsTemporaryEnemy(owner, true);
...
function CityBombard( wParam, lParam )
	...
	elseif selectedSite and selectedSite:CanRangeStrike() then
		if (selectedSite:CanRangeStrikeAt(plotX, plotY)) then
			Network.SendStrategicSiteCommand(StrategicSiteCommandTypes.SITECOMMAND_RANGE_SRIKE, selectedSitePlot:GetPlotIndex(), plot:GetPlotIndex());
		end
		UI.ClearSelectedCities();
    	UI.SetInterfaceMode(InterfaceModeTypes.INTERFACEMODE_SELECTION);
		return true;
...
function SupportAction( wParam, lParam )
	...
	if (plot ~= nil) then
		...
		local shiftKey = UIManager:GetShift();

		-- SEND SUPPORT ACTION
		if selectedUnit and selectedUnit:CanDoSupportActionAt(missionID, plotX, plotY) then
			Game.SelectionListGameNetMessage(GameMessageTypes.GAMEMESSAGE_PUSH_MISSION, missionID, plotX, plotY, 0, false, shiftKey);
		end
...
function ClearAllHighlights()
	...
	Events.ClearHexHighlightStyle(g_invisibleRevealWarning);
...
function MovementRButtonUp( wParam, lParam )
	...
	if pHeadSelectedUnit then
		...
		local isNeutralPlot = false;
		--if plot:GetOwner() == -1 then
			--isNeutralPlot = true;
		--end
--
		-- Is there someone here that we COULD bombard perhaps?
		-- Declare War check not done on neutral ground per Fluid Hostility
		if not isNeutralPlot then
			local eRivalTeam = pHeadSelectedUnit:GetDeclareWarRangeStrike(plot);
			if (eRivalTeam ~= -1) then
				UIManager:SetUICursor(0);
		...
		-- Set temporary enemies for Fluid Hostility
		if isNeutralPlot then
			-- Additional check to ensure we're only looking at ranged units. 
			if pHeadSelectedUnit:CanRangeStrikeAt(plotX, plotY, false, false) then
				local owner = pHeadSelectedUnit:GetOwner();
				local bestDefender = plot:GetBestDefender(-1, owner, pHeadSelectedUnit);
				if (bestDefender ~= nil and pHeadSelectedUnit:CanBeTemporaryEnemy(bestDefender:GetOwner())) then
					bestDefender:SetIsTemporaryEnemy(owner, true);
		...
		if (plot:IsVisibleEnemyUnit(pHeadSelectedUnit:GetOwner()) or plot:IsEnemyStrategicSite(pHeadSelectedUnit)) then		
...
function OnAddUnitMoveHexRangeHex(i, j, k, attackMove, unitID, playerType)
	if attackMove then
	...
	-- Overlay conditions
	local plotX : number, plotY : number = ToGridFromHex( i, j );
	local plot : object = Map.GetPlot(plotX,plotY);
	if (plot == nil) then
		return;
	end

	local unit : object = Players[playerType]:GetUnitByID(unitID);
	local teamType : number = unit:GetTeam();
	
	-- Invisibility reveal
	if (unit:HasInvisibility()) then
		if (plot:IsInvisibleVisibleToAnyEnemyTeam(team)) then
			Events.SerialEventHexHighlight( Vector2( i, j ), true, g_turn1Color, g_invisibleRevealWarning );
		end
	end
end
...
function InitializeWorldView()
	...
	InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_SUPPORT_ACTION][MouseEvents.PointerUp]		= SupportAction;
	...
	InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_SUPPORT_ACTION][MouseEvents.LButtonUp]		= SupportAction;
	...
	-- pointer...
	InterfaceModeMessageHandler[InterfaceModeTypes.INTERFACEMODE_ATTACK][MouseEvents.PointerUp]			= function(wParam,lParam) return false; end;
	...
	-- Fungal marvel effect borders
	local activatedFungalMinorMarvelPlotIndices : table = Game.GetActivatedFungalMinorMarvelPlotIndices();
	if(activatedFungalMinorMarvelPlotIndices == nil) then
		error("activatedFungalMinorMarvelPlotIndices was nil.");
	end

	for _,index : number in ipairs(activatedFungalMinorMarvelPlotIndices) do
		HighlightAffectedFungalMarvelPlots(index);
	end
