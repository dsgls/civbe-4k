...
function RefreshData()
	...
	for i,v in ipairs(potentialTradeSpots) do
		...
		tradeRoute.Resources= v.Resources;
		
		table.insert(g_Model, tradeRoute);
	end

	-- Highlight invalid trade plots
	local bHighlightMiasma : boolean = (not pPlayer:IsTradeRoutesMiasmaImmune()) and (pPlayer:GetGeneralUnitMiasmaHealthDelta() < 0);
...
function DisplayData()
	...
	for _, tradeRoute in ipairs(g_Model) do
		...
		CivIconHookup( tradeRoute.TargetPlayerId, 64, itemInstance.CivIcon, itemInstance.CivIconBG, nil, false, false, itemInstance.CivIconHighlight );
