...
function OnProdClick( cityID, prodName )
	...
	LuaEvents.LeaveEconomyOverview();
...
function UpdateDisplay()
    ...
    for pCity in pPlayer:Cities() do
        ...
        sortEntry.Energy = pCity:GetYieldRate( YieldTypes.YIELD_ENERGY );
        instance.Energy:SetText( sortEntry.Energy );
        
        sortEntry.Culture = pCity:GetCulturePerTurn();
...
function UpdateGPT()
    ...
    Controls.TotalGoldValue:SetText( Locale.ToNumber( pPlayer:GetEnergy(), "#.##" ) );
	...
	-- Have to add the Beacon amount here because it is variable based on the player's income
	local beaconEnergyDelta = pPlayer:GetBeaconEnergyCostPerTurn();
	local totalExpenses = pPlayer:CalculateInflatedCosts() + beaconEnergyDelta;

    Controls.TotalExpenseValue:SetText( "[COLOR_NEGATIVE_TEXT]" .. Locale.ToNumber( totalExpenses, "#.##" ) .. "[ENDCOLOR]" );
    ...
    for pCity in pPlayer:Cities() do
        ...
        local CityIncome = pCity:GetYieldRateTimes100(YieldTypes.YIELD_ENERGY) / 100;
    ...
    local iBaseGold = GameDefines.TRADE_ROUTE_BASE_ENERGY / 100;
    local iGoldPerPop = GameDefines.TRADE_ROUTE_CITY_POP_ENERGY_MULTIPLIER / 100;
    ...
    -- Improvements
    Controls.TileExpense:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_EO_EX_IMPROVEMENTS"));
    Controls.TileExpenseValue:SetText( Locale.ToNumber( pPlayer:GetImprovementGoldMaintenance(), "#.##" ) );
    
	-- Routes
	Controls.RouteExpense:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_EO_EX_ROUTES"));
    Controls.RouteExpenseValue:SetText( Locale.ToNumber( pPlayer:GetRouteEnergyMaintenance(), "#.##" ) );
	...
	-- Beacon operating expense	
	if (beaconEnergyDelta ~= 0) then
		Controls.BeaconExpense:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_TP_ENERGY_TO_BEACON", beaconEnergyDelta));
		Controls.BeaconExpenseValue:SetText( Locale.ToNumber( beaconEnergyDelta, "#.##" ) );		
	else
        Controls.BeaconExpense:SetHide(true);
    end
...
function SortFunction( a, b )
    ...
    if (entryA == nil) or (entryB == nil) then 
		...
		elseif( m_SortMode == eGold ) then
			valueA = entryA.Energy;
			valueB = entryB.Energy;
