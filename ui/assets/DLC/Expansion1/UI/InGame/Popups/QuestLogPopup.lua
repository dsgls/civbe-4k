...
--	CONSTANTS
-- ===========================================================================
local FilterType			= { QUEST = 1, VICTORY = 2, DOMESTIC = 3 };
...
function OnPopup(popupInfo)
	...
	if (popupInfo.Data1 ~= -1) then
		...
		elseif(m_CurrentExpandedQuestIndex == -1) then
			...
			local quest : object = player:GetQuestWithIndex(m_CurrentExpandedQuestIndex);
			if(quest == nil) then
				error("quest was nil.");
			end
			...
			if(questInfo.Domestic) then
				selectedTab = Controls.DomesticQuestsTab;
			else
				selectedTab = Controls.QuestsTab;
			end
		end
...
function Initialize()
	...
	m_tabs.AddTab( Controls.DomesticQuestsTab,		OnTabDomesticQuests );
...
-- ===========================================================================
function OnTabDomesticQuests( control )
	m_CurrentFilter = FilterType.DOMESTIC;
	UpdateWindow();
end
...
function UpdateWindow()
	...
	local numDomesticQuestsUpdated : number = player:GetNumDomesticQuestsMarkedUpdated();
	...
	if (numDomesticQuestsUpdated ~= 0) then
		Controls.DomesticQuestTabUpdatedBadge:SetHide(false);
		Controls.DomesticQuestTabUpdatedLabel:SetHide(false);
		Controls.DomesticQuestTabUpdatedLabel:SetText(numDomesticQuestsUpdated);
	else
		Controls.DomesticQuestTabUpdatedBadge:SetHide(true);
		Controls.DomesticQuestTabUpdatedLabel:SetHide(true);
	end
	...
	Controls.NoDomesticQuestsLabel:SetHide(m_CurrentFilter ~= FilterType.DOMESTIC);
	for i,quest in ipairs(quests) do
		...
		if (GameInfo.Quests[quest:GetType()].Domestic == true and m_CurrentFilter == FilterType.DOMESTIC) then
			Controls.NoQuestsLabel:SetHide(true);
			Controls.NoDomesticQuestsLabel:SetHide(true);
		end
	end
	...
	if IsScrollbarShowing( Controls.MainContentScrollPanel ) then
		...
		Controls.MainContentScrollPanel:SetSizeX(WIDTH_WITHOUT_SCROLL_BAR);
	end
...
function ShouldDisplayQuest(quest)
	...
	if ( m_CurrentFilter == FilterType.VICTORY ) then
		...
		local isCorrectQuestType : boolean = false;

		if( m_CurrentFilter == FilterType.DOMESTIC and
			questInfo.Domestic == true)
		then
			isCorrectQuestType = true;
		elseif(m_CurrentFilter == FilterType.QUEST and
			questInfo.Domestic == false and
			questInfo.Victory == false)
		then
			isCorrectQuestType = true;
		end

		if(isCorrectQuestType == true) then
			local isFinished	= not quest:IsInProgress();
			local isFailed		= isFinished and quest:DidFail();
