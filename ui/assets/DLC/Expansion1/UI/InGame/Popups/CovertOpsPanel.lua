...
local g_headerBlackMarketInstanceManager = InstanceManager:new( "HeaderBlackMarketInstance", "Header", Controls.BlackMarketStack );
local g_headerBlackMarketCityInstanceManager = InstanceManager:new( "HeaderBlackMarketInstance", "Header", Controls.CityStack );
local g_headerBlackMarketToggleCityInstanceManager = InstanceManager:new( "HeaderBlackMarketToggleInstance", "Header", Controls.CityStack );
...
local g_spyBlackMarketInstanceManager = InstanceManager:new( "SpyBlackMarketInstance", "Spy",	Controls.BlackMarketStack );
...
local g_blackMarketInstanceManager	= InstanceManager:new( "BlackMarketInstance", "BlackMarket", Controls.CityStack );
...
local g_blackMarketResource = -1;
local g_blackMarketBuying : boolean = true;	-- If set to true, the agent will sell to the black market instead
...
function UpdateWindow()	
	...
	g_headerBlackMarketInstanceManager:DestroyInstances();
	g_headerBlackMarketCityInstanceManager:DestroyInstances();
	g_headerBlackMarketToggleCityInstanceManager:DestroyInstances();
	g_headerCityInstanceManager:DestroyInstances();

	g_spyHQInstanceManager:DestroyInstances();
	g_spyBlackMarketInstanceManager:DestroyInstances();
	...
	g_blackMarketInstanceManager:DestroyInstances();
	...
	Controls.BlackMarketStack:DestroyAllChildren();
...
function UpdateWindow_Agents()	
	...
	header = g_headerBlackMarketInstanceManager:GetInstance();
	table.insert( g_uiListItems, header );
	...
	local blackMarketAgentNum = 0;

	MakeNationalSecurityProjectHeader();

	for i,agent in ipairs(agents) do
		if (not agent:IsDead()) then
			if ( agent:IsRunningBlackMarket() or agent:IsEstablishingBlackMarket()) then
				CreateBlackMarketAgent( agent );
				blackMarketAgentNum = blackMarketAgentNum + 1;
			elseif ( agent:CanTravel() and agent:GetCity() == nil ) then			
				CreateHQAgent( agent );
	...
	if (blackMarketAgentNum < 1) then
		noneInstance = {};
		ContextPtr:BuildInstanceForControl("NoneInstance", noneInstance, Controls.BlackMarketStack );
		table.insert( g_uiListItems, noneInstance );
	end
...
function UpdateWindow_Cities()	
	...
	-- Add the header for black market list
	ui = g_headerBlackMarketCityInstanceManager:GetInstance();
	local toggle = g_headerBlackMarketToggleCityInstanceManager:GetInstance();
	if (g_blackMarketBuying) then
		toggle.BuyModeButton:SetDisabled(true);
		toggle.BuySelected:SetHide(false);
		toggle.SellModeButton:RegisterCallback(Mouse.eLClick, function(void1, void2, button) ToggleBlackMarketBuySell(false); end);
	else
		toggle.SellModeButton:SetDisabled(true);
		toggle.SellSelected:SetHide(false);
		toggle.BuyModeButton:RegisterCallback(Mouse.eLClick, function(void1, void2, button) ToggleBlackMarketBuySell(true); end);
	end	
	
	table.insert( g_uiListItems, ui );
	table.insert( g_uiListItems, toggle );

	-- Build a list of resources for the black market
	local team : object = Teams[player:GetTeam()];
	local blackMarketResourceInfos : table = {};
	for resourceInfo : table in GameInfo.Resources() do
		if (resourceInfo.AvailableOnBlackMarket and team:IsResourceRevealed(resourceInfo.ID)) then
			table.insert(blackMarketResourceInfos, resourceInfo);
		end
	end
	
	-- Sort
	table.sort(blackMarketResourceInfos, function(a : table, b : table) return Locale.Compare(a.Description, b.Description) == -1; end);

	-- Set defaults
	agent:SetBlackMarketQuantity(1);
	for i : number, resourceInfo : table in ipairs(blackMarketResourceInfos) do
		ui = CreateBlackMarket(resourceInfo.ID, agent);
		table.insert(g_uiListItems, ui);
	end
	...
	RealizeScrollArea( OnAssignAgent, OnCancelAssignAgent );
end
...
-- ===========================================================================
--	Create Black Market
--	RETURNS: UI reference to black market	
-- ===========================================================================
function CreateBlackMarket( resourceType, agent )
	local resourceInfo : table = GameInfo.Resources[resourceType];
	local ui : table = g_blackMarketInstanceManager:GetInstance();

	ui.Name:SetText(Locale.Lookup(resourceInfo.Description));
	ui.BlackMarket:RegisterCallback(Mouse.eLClick, function(void1, void2, button) OnBlackMarketSelected(ui, resourceType ); end);

	-- Quantity controls
	ui.QuantityDown:RegisterCallback(Mouse.eLClick, function(void1, void2, button) 
		OnBlackMarketSelected(ui, resourceType);

		local newQuantity : number = ChangeBlackMarketQuantity(resourceType, -1);
		ui.Quantity:SetText(newQuantity);
		RefreshBlackMarketQuantityControls(ui, resourceType);		
	end);
	ui.QuantityUp:RegisterCallback(Mouse.eLClick, function(void1, void2, button) 
		OnBlackMarketSelected(ui, resourceType);
	...
	ui.QuantityDown:SetHide(true);
	ui.QuantityUp:SetHide(true);
	ui.QuantityGrid:SetHide(true);

	-- in BUYING mode
	if (g_blackMarketBuying) then
		if (agent:CanBuyOnBlackMarket(resourceType)) then
			ui.BlackMarket:SetDisabled(false);
			ui.Cost:SetText(Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_COST", agent:GetCapitalForResourceBuying(resourceType)));
		else
			ui.BlackMarket:SetDisabled(true);
			ui.Cost:SetText("[COLOR_RED]" .. Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_COST", agent:GetCapitalForResourceBuying(resourceType)));
		end
	else
	-- in SELLING mode
		if (agent:CanSellOnBlackMarket(resourceType)) then
			ui.BlackMarket:SetDisabled(false);
			ui.QuantityDown:SetHide(false);
			ui.QuantityUp:SetHide(false);
			ui.QuantityGrid:SetHide(false);
			RefreshBlackMarketQuantityControls(ui, resourceType, agent);
		else
			ui.BlackMarket:SetDisabled(true);
			ui.Cost:SetText("[COLOR_RED]" .. Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_NONE_UNAVAILABLE"));
		end
	end

	IconHookup(resourceInfo.PortraitIndex, 45, resourceInfo.IconAtlas, ui.Icon);

	return ui;
end

-- ===========================================================================
--	Refresh state of up/down buttons for increasing and decreasing quantity
--	RETURNS: None
-- ===========================================================================
function RefreshBlackMarketQuantityControls(ui : table, resourceType : number)
	local agent : object = g_agent;
	if (agent == nil) then
		return;
	end

	local current : number = agent:GetBlackMarketQuantity();
	local maxQuantity = agent:GetMaxBlackMarketResourcesSelling(resourceType);
	
	ui.QuantityUp:SetDisabled(false);
	ui.QuantityDown:SetDisabled(false);

	if (current >= maxQuantity) then
		current = maxQuantity;
		ui.QuantityUp:SetDisabled(true);
	end
	if (current <= 1) then
		ui.QuantityDown:SetDisabled(true);
	end

	ui.Quantity:SetText(current);

	local resourceInfo = GameInfo.Resources[resourceType];
	local absoluteMaxQuantity : number = agent:GetMaxBlackMarketResourcesSelling(-1);
	local tipStr : string = Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_MAX_SELLABLE", absoluteMaxQuantity, maxQuantity, resourceInfo.IconString, resourceInfo.Description);
	ui.QuantityUp:SetToolTipString(tipStr);
	ui.QuantityDown:SetToolTipString(tipStr);
	ui.Quantity:SetToolTipString(tipStr);

	local totalCost : number = agent:GetCapitalForResourceSelling(resourceType) * current;
	ui.Cost:SetText(Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_COST", totalCost));
end

-- ===========================================================================
--	Increase or Decrease an agent's current Black Market quantity
--	RETURNS: New current quantity;
-- ===========================================================================
function ChangeBlackMarketQuantity(resourceType : number, delta : number)
	local agent : object = g_agent;
	if (agent == nil) then
		return;
	end
	...
	if (g_blackMarketBuying) then
		maxValue = agent:GetMaxBlackMarketResourcesBuying(resourceType);
	else
		maxValue = agent:GetMaxBlackMarketResourcesSelling(resourceType);
	end
	local newValue : number = current;
	...
	if (newValue >= 1 and newValue <= maxValue) then
		agent:SetBlackMarketQuantity(newValue);
		return newValue;
	else
		return current;
	end
...
-- ===========================================================================
--	Toggle Black Market Buy/Sell
--	RETURNS: None
-- ===========================================================================
function ToggleBlackMarketBuySell(buying : boolean)
	if (buying ~= g_blackMarketBuying) then
		g_blackMarketBuying = buying;
		UpdateWindow();
	end
end
...
function CreateBlackMarketAgent( agent )
	if ( agent:IsDead() ) then
		return;
	end

	local ui : table = g_spyBlackMarketInstanceManager:GetInstance();
	...
	local resourceType : number = agent:GetBlackMarketResource();
	local resourceInfo : table = GameInfo.Resources[resourceType];		

	--ui.ResourceName:SetText(resourceInfo.IconString .. Locale.Lookup(resourceInfo.Description));
	IconHookup(resourceInfo.PortraitIndex, 45, resourceInfo.IconAtlas, ui.ResourceIcon);

	SetProgressBar( ui, agent );
	ui.Assign		:SetText( Locale.ConvertTextKey( "{TXT_KEY_COVERT_ABORT:upper}" ));
	ui.Assign		:SetVoid1( ui.id );
	ui.Assign		:RegisterCallback( Mouse.eLClick, OnAbortBlackMarket );	

	if (agent:IsEstablishingBlackMarket()) then
		ui.ProgressBar:SetHide(false);
		local isBuying : boolean = agent:IsEstablishingBlackMarketBuy();
		local isSelling : boolean = agent:IsEstablishingBlackMarketSell();
		if (isBuying) then
			ui.Action:SetText( Locale.ConvertTextKey( "TXT_KEY_COVERT_ESTABLISHING_BLACK_MARKET_BUYING" , resourceInfo.IconString, resourceInfo.Description ));
		else
			ui.Action:SetText( Locale.ConvertTextKey( "TXT_KEY_COVERT_ESTABLISHING_BLACK_MARKET_SELLING" , resourceInfo.IconString, resourceInfo.Description ));
		end
		ui.Assign		:SetText( Locale.ConvertTextKey( "{TXT_KEY_COVERT_ABORT:upper}" ));
		ui.Effect:SetHide(true);
		ui.EffectLabel:SetHide(true);
	else
		ui.ProgressBar:SetHide(true);
		ui.Action:SetHide(true);
		ui.Effect:SetHide(false);
		ui.EffectLabel:SetHide(false);
		local isBuying : boolean = agent:IsBlackMarketBuying();
		local isSelling : boolean = agent:IsBlackMarketSelling();
		if (isBuying) then
			ui.EffectLabel:SetText(Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_BUYING_EFFECT", agent:GetBlackMarketQuantity(), resourceInfo.IconString, resourceInfo.Description, agent:GetCapitalForResourceBuying(resourceType)));
		elseif (isSelling) then
			ui.EffectLabel:SetText(Locale.ConvertTextKey("TXT_KEY_COVERT_BLACK_MARKET_SELLING_EFFECT", agent:GetBlackMarketQuantity(), resourceInfo.IconString, resourceInfo.Description, agent:GetCapitalForResourceSelling(resourceType) * agent:GetBlackMarketQuantity()));
		else
			-- error
			print("**** ERROR: CovertOps ****   Agent assigned to black market not properly flagged for buying/selling: ", agent, agent:GetName() );
			return;
		end
	end
...
function SetProgressBar( ui, agent )
	...
	local turnsLeft		= math.max(goal - progress, 1);
...
function OnAbortBlackMarket(void1, void2, button)
	if (not IsMyTurn()) then return end;
...
function OnSelectCity( ui, city )
	...
	g_blackMarketResource = -1;
	g_blackMarketBuying = true;
	g_city = city;
	OnShowCity( city ); 			
end

-- ===========================================================================
--	An resource is selected from the list.
-- ===========================================================================
function OnBlackMarketSelected( ui, resourceType )
	if (not IsMyTurn()) then return end;
	...
	-- If resource is different, reset quantity as well
	if (resourceType ~= g_blackMarketResource) then
		g_blackMarketResource = resourceType;

		-- Re-establish visible quantity for this black market as the stored quantity
		local curQuantity : number = tonumber(ui.Quantity:GetText());
		if (curQuantity ~= nil and curQuantity >= 1) then
			g_agent:SetBlackMarketQuantity(curQuantity);
		else
			g_agent:SetBlackMarketQuantity(1);
...
function OnAssignAgent()
	...
	elseif (g_blackMarketResource ~= -1) then
		g_agent:EstablishBlackMarket(g_blackMarketResource, g_blackMarketBuying);
	end
	...
	if IsScrollbarShowing( Controls.MainScrollPanel ) then		
		for i,ui in ipairs(g_uiListItems) do
			...
			elseif ( ui.Project ~= nil ) then
				...
				ui.Selected:SetSizeX( ART_LIST_ITEM_W_SCROLLBAR_WIDTH );
			elseif ( ui.BlackMarket ~= nil ) then
				ui.BlackMarket:SetSizeX( ART_LIST_ITEM_W_SCROLLBAR_WIDTH );
				ui.Selected:SetSizeX( ART_LIST_ITEM_W_SCROLLBAR_WIDTH );
