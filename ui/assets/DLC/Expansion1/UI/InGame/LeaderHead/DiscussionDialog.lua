...
local g_iFavorDiscussionType;
local g_iFavorDiscussionTarget;
...
function LeaderMessageHandler( iPlayer, iDiploUIState, szLeaderMessage, iAnimationAction, iData1 )
	...
	CivIconHookup( g_iAIPlayer, 45, Controls.CivIcon, Controls.CivIconBG, nil, false, false, Controls.CivIconHighlight );
	...
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_CONFRONT_YOU_KILLED_MY_SPY) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_COVERT_OPS_BOMBSHELL) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_STOP_DIGGING) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_SPREAD_MIASMA) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_REMOVE_MIASMA) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_ANTAGONIZE_ALIENS) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_ORBITAL_DEPLOYMENT) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_STATION_AGGRESSION) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_SPREAD_MIASMA_BROKEN_PROMISE) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_REMOVE_MIASMA_BROKEN_PROMISE) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_ANTAGONIZE_ALIENS_BROKEN_PROMISE) then
		bMyMode = true;
	elseif (iDiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_ORBITAL_DEPLOYMENT_BROKEN_PROMISE) then
		bMyMode = true;
    ...
    if (bMyMode) then
		...
		--Controls.LeaderSpeechFrame:SetSizeY( contentSize );
		...
		g_iFavorDiscussionType = pAIPlayer:GetLastDiploEventType();
		g_iFavorDiscussionTarget = pAIPlayer:GetLastDiploTargetPlayer();

		local bPursueFavor : boolean = false;
		if g_iFavorDiscussionType ~= nil and g_iFavorDiscussionType >= 0 then
			-- See if we have enough favors to pursue this
			
			local numFavors = activePlayer:GetNumFavors(iPlayer);
			local numFavorsNeeded = pAIPlayer:GetNumFavorsToForceEvent( iActivePlayer, g_iFavorDiscussionType, -1, g_iFavorDiscussionTarget );
			if numFavors >= numFavorsNeeded then
				bPursueFavor = true;
			end			
		end

		-- Human invoked discussion
		if (bPursueFavor) then

			local numFavorsNeeded = pAIPlayer:GetNumFavorsToForceEvent( iActivePlayer, g_iFavorDiscussionType, -1, g_iFavorDiscussionTarget );

			strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_USE_FAVORS", numFavorsNeeded );
			strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_NEW_DISCUSSION" );

		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
			
			-- Discussion Root Mode
			if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then

				local bHasAgents = activePlayer:GetNumCovertAgents() > 0;
				local bHasBeenSpiedOn = activePlayer:HasBeenSpiedOnByPlayer(iPlayer);

				if (activePlayer:GetOtherPlayerTurnOrbitalDeployment(g_iAIPlayer) > 0 and not pAIPlayer:IsPlayerAskedNotToOrbitalDeployment(iActivePlayer)) then
					strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_STOP_ORBITAL_DEPLOYMENT" );
				end

				if (not pAIPlayer:IsStopSpyingMessageTooSoon(iActivePlayer) and bHasAgents and bHasBeenSpiedOn and (g_iAITeam ~= Game.GetActiveTeam())) then
					strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_STOP_COVERT_OPS" );
				end

				if (activePlayer:GetPlayerRelativeAlienAggression(g_iAIPlayer) >= 30 and not pAIPlayer:IsPlayerAskedNotToAntagonizeAliens(iActivePlayer)) then
					strButton3Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_STOP_ANTAGONIZE_ALIENS" );
				end
				...
				if (pAIPlayer:GetTeam() ~= Players[iActivePlayer]:GetTeam()) then
					...
					if (not pAIPlayer:IsCoopAgreement(iActivePlayer) and not pAIPlayer:IsCoopAgreementMessageTooSoon(iActivePlayer)) then
						strButton6Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_COOP_AGREEMENT" );
						strButton6Tooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_MESSAGE_COOP_AGREEMENT_TT" );
					-- Tell the player we're done working with him
					--elseif (pAIPlayer:IsCoopAgreement(iActivePlayer)) then
					...
					if(pAIPlayer:GetNickName() ~= "" and Game:IsNetworkMultiPlayer()) then
						...
						strLeaderName = TruncateStringByLength(pAIPlayer:GetNickName(), MAX_NAME_CHARS);
					else
						strLeaderName = pAIPlayer:GetName();
					end
					
					-- Make sure the minimum time has elapsed for us to condemn again
					if (activePlayer:GetDenouncedPlayerCounter(iPlayer) < 0) then
		...
		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_CONFRONT_YOU_KILLED_MY_SPY) then
			...
			if(not activePlayer:IsDenouncedPlayer(iPlayer)) then
				strButton3Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_CAUGHT_DENOUNCE_FOR_SPYING" );
 			end
 			
			strButton4Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_CAUGHT_DECLARE_WAR" );
		...
		-- Beyond Earth additions
		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_SPREAD_MIASMA) then
			strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DONT_STOP_SPREAD_MIASMA" );
			strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_STOP_SPREAD_MIASMA" );
			bHideBackButton = true;
	    
		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_REMOVE_MIASMA) then
			strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DONT_STOP_REMOVE_MIASMA" );
			strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_STOP_REMOVE_MIASMA" );
			bHideBackButton = true;

		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_ANTAGONIZE_ALIENS) then
			strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DONT_STOP_ANTAGONIZE_ALIENS" );
			strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_STOP_ANTAGONIZE_ALIENS" );
			bHideBackButton = true;

		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_ORBITAL_DEPLOYMENT) then
			strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DONT_STOP_ORBITAL_DEPLOYMENT" );
			strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_STOP_ORBITAL_DEPLOYMENT" );
			bHideBackButton = true;

		elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_STATION_AGGRESSION) then
			strButton1Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_DONT_STOP_STATION_AGGRESSION" );
			strButton2Text = Locale.ConvertTextKey( "TXT_KEY_DIPLO_DISCUSS_STOP_STATION_AGGRESSION" );
			bHideBackButton = true;
		...
		Controls.TalkOptionsFrame:ReprocessAnchoring();
	end
...
function OnPopup(popupInfo)	
	if( popupInfo.Type ~= ButtonPopupTypes.BUTTONPOPUP_END_MENU ) then		
	    return;
	end	
...
function OnButton1()
	...
	-- Favor-force follow up (fa fa fa fa fa)
	-- Human is using Favors to pursue the last discussion topic
	if g_iFavorDiscussionType ~= nil and g_iFavorDiscussionType >= 0 then

		--local iAgainstPlayer = pAIPlayer:GetLastDiploTargetPlayer();
		-- Handle a Favor-forced event
		Game.DoFromUIDiploEvent( g_iFavorDiscussionType, g_iAIPlayer, iButtonID, g_iFavorDiscussionTarget, true );

    -- Human-invoked discussion
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then

		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			-- Ask the AI player not to deploy orbital units against us
			Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_STOP_ORBITAL_DEPLOYMENT, g_iAIPlayer, 0, 0 );
		end
    ...
    -- Civ BE	
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_SPREAD_MIASMA) then
	   Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WARN_SPREAD_MIASMA, g_iAIPlayer, iButtonID, iAgainstPlayer);
	   
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_REMOVE_MIASMA) then
	   Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WARN_REMOVE_MIASMA, g_iAIPlayer, iButtonID, iAgainstPlayer);
	   
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_ANTAGONIZE_ALIENS) then
	   Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WARN_ANTAGONIZE_ALIENS, g_iAIPlayer, iButtonID, iAgainstPlayer);
	   
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_ORBITAL_DEPLOYMENT) then
	   Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WARN_ORBITAL_DEPLOYMENT, g_iAIPlayer, iButtonID, iAgainstPlayer);

	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_WARN_STATION_AGGRESSION) then
	   Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WARN_STATION_AGGRESSION, g_iAIPlayer, iButtonID, iAgainstPlayer);
...
function OnButton2()
	...
	-- Human had the option to use Favors to pursue the last discussion, but has decided to pass
	if g_iFavorDiscussionType ~= nil and g_iFavorDiscussionType >= 0 then
		
		-- Restart the discussion process
...
function OnButton3()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			-- Ask the AI player not to antagonize aliens  
			Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_STOP_ANTAGONIZE_ALIENS, g_iAIPlayer, 0, 0 );
		end
...
function OnButton6()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		...
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			-- Ask the AI player to work together
			if (not pAIPlayer:IsCoopAgreementMessageTooSoon(iActivePlayer)) then
