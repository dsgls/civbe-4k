...
g_TechFilters.TECHFILTER_RECOMMENDED = function(techInfo)
	for advisor : number = 0, AdvisorTypes.NUM_ADVISOR_TYPES - 1, 1 do
		if Game.IsTechRecommended(techInfo.ID, advisor) then
			return true;
		end
	end
...
-- Capital Filter
g_TechFilters.TECHFILTER_CAPITAL = function(techInfo)
	local yieldType : string = "YIELD_CAPITAL";
...
g_TechFilters.TECHFILTER_AQUATIC = function(techInfo)
	-- Sea-domain Units
	local condition : string = "PrereqTech = '" .. techInfo.Type .. "' and Domain = 'DOMAIN_SEA'";
	for row in GameInfo.Units(condition) do
		if row ~= nil then
			return true;
		end
	...
	-- Sea-domain Improvements (based on Worker Builds)
	condition = "TechType = '" .. techInfo.Type .. "'";
	for row in GameInfo.Improvement_TechYieldChanges(condition) do
		local subCondition : string = "ImprovementType = '" .. row.ImprovementType .. "' and Water = 1";
		for subRow in GameInfo.Builds(subCondition) do
			if subRow ~= nil then
				return true;
			end
		end
	...
	-- Water Buildings
	condition = "PrereqTech = '" .. techInfo.Type .. "' and (WaterOnly = 1 OR WaterAccess = 1)";
	...
	-- Water abilities
	if (techInfo.AllowsDeepWaterCityMovement)
	then
		return true;
	end
