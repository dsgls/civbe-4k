	...
	if (m_iLeague ~= nil and m_iProject ~= nil) then
		...
		if (pLeague ~= nil) then
			...
			if (tProject.ProjectSplashImage ~= nil) then
				Controls.BackgroundImage:SetTexture(tProject.ProjectSplashImage);
			end
	...
	if (tProject ~= nil) then
		...
		elseif (iRewardTier == 0) then
			...
			for i = 1, iRewardTier, 1 do
				sTooltip = sTooltip .. "/n[ICON_BULLET]";
	...
	if (m_iLeague ~= nil and m_iProject ~= nil) then
		...
		if (pLeague ~= nil) then
			...
			elseif (not pTeam:IsHasMet(Game.GetActiveTeam())) then
				...
				IconHookup( 24, 64, "CIV_COLOR_ATLAS", controlTable.Portrait );
			-- Met players
			else
			...
			if (iTier >= 2) then
				...
				strRewardTT = pLeague:GetProjectRewardTierDetails(2, m_iProject) .. "/n/n" .. strRewardTT;
			end
			if (iTier >= 3) then
				strRewardIcons = strRewardIcons .. "[ICON_TROPHY_GOLD]";
				strRewardTT = pLeague:GetProjectRewardTierDetails(3, m_iProject) .. "/n/n" .. strRewardTT;
			end
