...
--
function GetStatsIconStrings( movement:number, strength:number, rangedStrength:number, deltaMovement:number, deltaStrength:number, deltaRangedStrength:number, isShowingStats:boolean )
	...
	return movementString, strengthString, rangedStrengthString;
end
...
function UpdateCurrentUpgrade()
	...
	if m_selectedUpgrade ~= nil then
		...
		movementString, strengthString, rangedStrengthString =  GetStatsIconStrings( 
		...
		-- Grab strings
		movementString, strengthString, rangedStrengthString =  GetStatsIconStrings( m_unit.Movement, m_unit.Strength, m_unit.RangedStrength, 0, 0, 0, false );
	end

	if (m_unit ~= nil) then
		rangeString = "[ICON_ATTACK_RANGE]"..m_unit.Range;
	end
	...
	Controls.Range:SetToolTipString( Locale.ConvertTextKey("TXT_KEY_UPANEL_ATTACK_RANGE_TT", m_unit.Range));
	Controls.Range:SetHide( m_unit.Range == 0 );
...
function GetPerkStats( unitInfo:table )
	...
	if DomainTypes[unitInfo.Domain] == DomainTypes.DOMAIN_AIR then
		movement			= m_player:GetBaseRangeWithPerks(unitInfo.ID);							
		local rebaseRange	= movement * GameDefines.AIR_UNIT_REBASE_RANGE_MULTIPLIER;
		rebaseRange			= rebaseRange / 100;
		movementTip			= Locale.ConvertTextKey("TXT_KEY_UPANEL_UNIT_MAY_STRIKE_REBASE", movement, rebaseRange);
	...
	return movement, strength, rangedStrength, movementTip;
end
...
function UpdateSingleUnit( unitInfo:table )
	...
	movement, strength, rangedStrength, movementTip = GetPerkStats( unitInfo );
