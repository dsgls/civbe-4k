...
local m_OrbitalIM = InstanceManager:new( "UnitInstance", "Root", Controls.OrbitalStack );
...
function BuildUnitList()
    ...
    do
        ...
        if unit:IsOrbitalUnit() then
			instance = m_OrbitalIM:GetInstance();
			bFoundOrbital = true;
		elseif( unit:GetUnitCombatType() ~= -1 ) then
		...
		if( unit:IsOrbitalUnit() ) then
			sortEntry.status = "TXT_KEY_UNIT_STATUS_ORBITAL";
        ...
        elseif( unit:IsAutomated()) then
			local automateType = unit:GetAutomateType();
			local automate = GameInfo.Automates[automateType];
			if(automate ~= nil) then
				sortEntry.status = automate.Description;
				instance.Status:SetHide( false );
		...
		if not unit:IsOrbitalUnit() then
			...
			-- Orbital units show no movement
			instance.MovementPip:SetHide(true);
			instance.Movement:SetHide(true);
		end
        
        sortEntry.strength = unit:GetCombatStrength();
        ...
        sortEntry.ranged = unit:GetRangedCombatStrength();
        if( sortEntry.ranged == 0 or not unit:IsRanged() ) then
	...
	local showCivilianSeparator : boolean = bFoundMilitary and bFoundCivilian;
	local showOrbitalSeparator : boolean = (bFoundMilitary or bFoundCivilian) and bFoundOrbital;

	Controls.CivilianSeperator:SetHide( showCivilianSeparator );
	Controls.OrbitalSeperator:SetHide( showOrbitalSeparator );
    
    Controls.MilitaryStack:CalculateSize();
    Controls.CivilianStack:CalculateSize();
	Controls.OrbitalStack:CalculateSize();
