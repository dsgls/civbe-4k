...
function GetProposals(league, activePlayerId)
	...
	local GetChoicesForResolution = function(resolutionType) 
		...
		if(decisionType ~= "RESOLUTION_DECISION_YES_OR_NO") then
			...
			for _, choiceId in ipairs(choices) do
				...
				if(decisionType == "RESOLUTION_DECISION_MAJOR_CIV_MEMBER") then
	...
	for i,v in ipairs(enactProposalsOnHold) do
		...
		local proposal = {
			...
			ToolTip = "[COLOR_WARNING_TEXT]" .. Locale.Lookup("TXT_KEY_LEAGUE_OVERVIEW_PROPOSAL_ON_HOLD_TT") .. "[ENDCOLOR]/n/n" .. league:GetResolutionDetails(v.Type, activePlayerId, v.ID, choice);
			OnHold = true;
		};
